...
- Recovery drill: restore Postgres and the private partner-asset snapshot into isolated targets. Run `npm run recovery:verify` for the database proof, then set `SANDFEST_RECOVERY_ASSET_DIR` and run `npm run recovery:verify:assets` to prove every restored sponsor/vendor upload by size and SHA-256 before recording either drill timestamp.
