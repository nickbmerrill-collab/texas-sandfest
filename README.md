...
- On the presentation Mac, `npm run board:service:start` installs a checkout-owned launchd keepalive around the same guarded board stack. Status, restart, and stop commands fail closed if the service label belongs to another checkout, and ordinary `npm run board:stop` retires the verified keepalive before signaling the supervisor so stale source cannot silently respawn after an apparently successful stop.
