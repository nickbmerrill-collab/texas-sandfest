...
- Recovery drill: restore Postgres and the private upload-disk snapshot into isolated targets. `npm run recovery:verify` uses one repeatable-read transaction to require the current 74-column schema, four governed configuration documents, 15 operational ledgers, and exact event alignment before emitting a content-bound aggregate database manifest SHA-256. Then set `SANDFEST_RECOVERY_ASSET_DIR` and run `npm run recovery:verify:assets` to prove every restored sponsor, vendor, and incoming-document upload by size and SHA-256 before recording either drill timestamp.
