...
- Private document intake (`lib/incoming-documents.mjs`): operations staff can upload board packets, provider exports, finance files, runbooks, and communications into an annual review queue with domain and owner assignment, text/CSV/JSON previews, checksum deduplication, review and archive states, and audited checksum-verified downloads. File bytes remain on a private persistent mount; Postgres stores governed metadata only, API responses never expose storage paths, and production readiness requires `SANDFEST_INCOMING_DOCUMENT_DIR`.
