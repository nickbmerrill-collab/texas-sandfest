...
function renderAdminVendorReadiness(payload) {
  ...
  target.innerHTML = vendorApplications.map(application => {
    ...
    return `<article class="admin-vendor-account" data-admin-vendor="${escapeAttr(application.id)}" data-status="${escapeAttr(state.status || "blocked")}">
      ...
      <div class="admin-vendor-readiness-strip"><span>${state.compliance?.approved || 0}/${state.compliance?.required || requirements.length} compliance</span><span>${escapeHtml(conditionLabel(state.profileStatus || "missing"))} profile</span><span>${escapeHtml(conditionLabel(state.assignmentStatus || "unassigned"))} load-in</span>${assignment.boothNumber ? `<span>${escapeHtml(`${assignment.boothNumber}${assignment.loadInStart ? ` · ${portalDate(assignment.loadInStart)}` : ""}`)}</span>` : ""}</div>
