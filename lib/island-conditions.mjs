...
const INCIDENT_MESSAGE_STATUSES = ["not_requested", "draft_ready", "approved", "queued", "sending", "sent", "failed", "dismissed", "canceled"];
const INCIDENT_DELIVERY_RESOLUTIONS = ["confirmed_sent", "confirmed_not_sent"];
...
function normalizeIncidentNotification(input) {
  ...
  return {
    ...
    deliveryIdempotencyKey: text(notification.deliveryIdempotencyKey, 100) || null,
    deliveryClaimId: text(notification.deliveryClaimId, 100) || null,
    deliveryClaimedAt: iso(notification.deliveryClaimedAt, null),
    providerSubmissionStartedAt: iso(notification.providerSubmissionStartedAt, null),
    deliveryQueueVersion: Math.max(0, Math.round(boundedNumber(notification.deliveryQueueVersion, 0, 10_000, 0))),
    deliveryOutcomeUnknown: notification.deliveryOutcomeUnknown === true,
    deliveryResolution: INCIDENT_DELIVERY_RESOLUTIONS.includes(notification.deliveryResolution) ? notification.deliveryResolution : null,
    deliveryReconciledAt: iso(notification.deliveryReconciledAt, null),
    deliveryReconciledBy: text(notification.deliveryReconciledBy, 120) || null,
    deliveryResolutionNote: text(notification.deliveryResolutionNote, 1000) || null,
...
export function incidentDispatchDeliveryJobKey(dispatchInput) {
  const dispatch = normalizeIncidentDispatch(dispatchInput);
  if (!dispatch || dispatch.notification.status !== "queued") return null;
  return `incident-dispatch:${dispatch.id}:v${Math.max(1, dispatch.notification.deliveryQueueVersion)}`;
}
...
function incidentCreationMatches(existing, candidate) {
  return existing.sourceType === candidate.sourceType
    ...
    && existing.ownerTeam === candidate.ownerTeam
    && existing.ownerName === candidate.ownerName
    && existing.publicImpact === candidate.publicImpact
    && existing.publicAlertRecommended === candidate.publicAlertRecommended
    && existing.latestSignalAt === candidate.latestSignalAt
    && existing.latestLevel === candidate.latestLevel
    && JSON.stringify(existing.latestMetrics) === JSON.stringify(candidate.latestMetrics)
    && existing.createdBy === candidate.createdBy
    && existing.timeline[0]?.note === candidate.timeline[0]?.note;
}
...
export function createOperationsIncident(docInput, input, options = {}) {
  ...
  const existing = doc.incidents.find(item => item.id === incident.id);
  if (existing) {
    return incidentCreationMatches(existing, incident)
      ? { ok: true, changed: false, duplicate: true, replay: true, incident: existing, doc }
      : { ok: false, code: "IDEMPOTENCY_CONFLICT", error: "This incident request key was already used for different incident details." };
  }
  const duplicate = activeIncidentFor(doc, sourceType, source);
  if (duplicate) return { ok: true, changed: false, duplicate: true, replay: false, incident: duplicate, doc };
  return {
    ok: true, changed: true, duplicate: false, replay: false, incident,
...
export function updateOperationsIncident(docInput, incidentId, patch, options = {}) {
  ...
  const dispatches = closed ? doc.dispatches.map(dispatch => {
    ...
    const providerSubmissionInFlight = dispatch.notification.status === "sending"
      && Boolean(dispatch.notification.providerSubmissionStartedAt);
    return normalizeIncidentDispatch({
      ...
      notification: {
        ...
        status: ["sent", "dismissed"].includes(dispatch.notification.status) || providerSubmissionInFlight
          ? dispatch.notification.status
          : "canceled",
        deliveryClaimId: providerSubmissionInFlight ? dispatch.notification.deliveryClaimId : null,
        deliveryClaimedAt: providerSubmissionInFlight ? dispatch.notification.deliveryClaimedAt : null,
        providerSubmissionStartedAt: providerSubmissionInFlight ? dispatch.notification.providerSubmissionStartedAt : null,
        lastError: providerSubmissionInFlight
          ? "Incident closed while provider submission was in progress; verify delivery."
          : dispatch.notification.deliveryOutcomeUnknown ? dispatch.notification.lastError : null
      },
...
export function updateIncidentDispatch(docInput, dispatchId, patch, options = {}) {
  ...
  if (messageChanged && ["queued", "sending", "sent"].includes(current.notification.status)) return { ok: false, error: "Queued, sending, or sent messages cannot be edited." };
  ...
  const providerSubmissionInFlight = current.notification.status === "sending"
    && Boolean(current.notification.providerSubmissionStartedAt);
  const notification = normalizeIncidentNotification({
    ...current.notification,
    subject,
    body,
    status: closed && !["sent", "dismissed"].includes(current.notification.status) && !providerSubmissionInFlight
      ? "canceled"
    ...
    deliveryClaimId: closed && !providerSubmissionInFlight ? null : current.notification.deliveryClaimId,
    deliveryClaimedAt: closed && !providerSubmissionInFlight ? null : current.notification.deliveryClaimedAt,
    providerSubmissionStartedAt: closed && !providerSubmissionInFlight ? null : current.notification.providerSubmissionStartedAt,
    lastError: providerSubmissionInFlight
      ? "Dispatch closed while provider submission was in progress; verify delivery."
      : messageChanged ? null : current.notification.lastError
  });
  const dispatch = normalizeIncidentDispatch({
    ...current,
...
export function reviewIncidentDispatchMessage(docInput, dispatchId, action, options = {}) {
  ...
  if (current.notification.deliveryOutcomeUnknown) {
    return { ok: false, conflict: true, outcomeUnknown: true, error: "Verify the provider outcome before reviewing or dismissing this message." };
  }
...
export function queueIncidentDispatchMessage(docInput, dispatchId, options = {}) {
  ...
  if (current.notification.status === "queued") {
    const recipient = resolveIncidentDispatchRecipient(doc, dispatchId, options);
    if (!recipient.ok) return recipient;
    return { ok: true, changed: false, duplicate: true, dispatch: current, recipient, doc };
  }
  ...
  if (!recipient.ok) return recipient;
  const now = options.now ?? new Date().toISOString();
  const existingIdempotencyKey = text(current.notification.deliveryIdempotencyKey, 100);
  ...
  const dispatch = normalizeIncidentDispatch({
    ...
    notification: {
      ...
      deliveryQueueVersion: current.notification.deliveryQueueVersion + 1,
  ...
  const dispatches = doc.dispatches.slice();
  dispatches[index] = dispatch;
  return { ok: true, changed: true, duplicate: false, dispatch, recipient, doc: { ...doc, lastUpdated: now, dispatches } };
}

export function claimIncidentDispatchDelivery(docInput, dispatchId, options = {}) {
  const doc = normalizeIslandConditions(docInput);
  const index = doc.dispatches.findIndex(item => item.id === sourceId(dispatchId));
  if (index < 0) return { ok: false, canceled: true, status: "missing", error: "Dispatch not found." };
  const current = doc.dispatches[index];
  if (current.notification.status !== "queued") {
    return {
      ok: false,
      canceled: ["failed", "dismissed", "canceled"].includes(current.notification.status),
      status: current.notification.status,
      error: `Dispatch email is ${current.notification.status}, not queued.`
    };
  }
  ...
  const dispatch = normalizeIncidentDispatch({
    ...
    updatedAt: now,
    updatedBy: text(options.actorId, 120) || "worker"
  });
  const dispatches = doc.dispatches.slice();
  dispatches[index] = dispatch;
  return { ok: true, dispatch, recipient: recipient.recipient, toName: recipient.toName, doc: { ...doc, lastUpdated: now, dispatches } };
}

export function beginIncidentDispatchProviderSubmission(docInput, dispatchId, options = {}) {
  const doc = normalizeIslandConditions(docInput);
  const index = doc.dispatches.findIndex(item => item.id === sourceId(dispatchId));
  if (index < 0) return { ok: false, canceled: true, status: "missing", error: "Dispatch not found." };
  const current = doc.dispatches[index];
  if (current.notification.status !== "sending") {
    return {
      ok: false,
      canceled: ["failed", "dismissed", "canceled"].includes(current.notification.status),
      status: current.notification.status,
      error: `Dispatch email is ${current.notification.status}, not sending.`
    };
  }
  const deliveryClaimId = text(options.deliveryClaimId, 100);
  if (!deliveryClaimId || deliveryClaimId !== current.notification.deliveryClaimId) {
    return { ok: false, canceled: true, status: current.notification.status, error: "Delivery claim no longer owns this dispatch." };
  }
  if (current.notification.providerSubmissionStartedAt) {
    return { ok: false, outcomeUnknown: true, status: current.notification.status, error: "Provider submission already started; verify delivery before retrying." };
  }
  const recipient = resolveIncidentDispatchRecipient(doc, dispatchId, options);
  if (!recipient.ok) return recipient;
  const now = options.now ?? new Date().toISOString();
  const dispatch = normalizeIncidentDispatch({
    ...current,
    notification: { ...current.notification, providerSubmissionStartedAt: now },
    updatedAt: now,
    updatedBy: text(options.actorId, 120) || "worker"
  });
  const dispatches = doc.dispatches.slice();
  dispatches[index] = dispatch;
  return { ok: true, changed: true, dispatch, recipient: recipient.recipient, toName: recipient.toName, doc: { ...doc, lastUpdated: now, dispatches } };
}
...
export function recordIncidentDispatchDelivery(docInput, dispatchId, delivery, options = {}) {
  ...
  if (index < 0) return { ok: false, error: "Dispatch not found." };
  const current = doc.dispatches[index];
  if (!["queued", "sending"].includes(current.notification.status)) return { ok: false, error: "Dispatch email is not queued or sending." };
  const deliveryClaimId = text(options.deliveryClaimId, 100);
  if (current.notification.status === "sending" && (!deliveryClaimId || deliveryClaimId !== current.notification.deliveryClaimId)) {
    return { ok: false, error: "Delivery claim no longer owns this dispatch." };
  }
  const now = options.now ?? new Date().toISOString();
  const sent = delivery?.sent === true;
  const terminal = options.terminal === true;
  if (current.notification.status === "sending" && !current.notification.providerSubmissionStartedAt && (sent || !terminal)) {
  ...
  const notification = normalizeIncidentNotification({
    ...
    deliveryClaimId: null,
    deliveryClaimedAt: null,
    providerSubmissionStartedAt: sent || terminal ? current.notification.providerSubmissionStartedAt : null,
...
export function reconcileIncidentDispatchDelivery(docInput, dispatchId, input, options = {}) {
  const doc = normalizeIslandConditions(docInput);
  const index = doc.dispatches.findIndex(item => item.id === sourceId(dispatchId));
  if (index < 0) return { ok: false, error: "Dispatch not found." };
  const current = doc.dispatches[index];
  if (!current.notification.deliveryOutcomeUnknown) {
    return { ok: false, conflict: true, error: "This dispatch does not have an unknown provider outcome." };
  }
  const action = INCIDENT_DELIVERY_RESOLUTIONS.includes(input?.action) ? input.action : null;
  ...
  const provider = current.notification.provider && current.notification.provider !== "worker"
    ? current.notification.provider
    : "brevo";
  const closed = ["completed", "canceled"].includes(current.status)
    || ["dismissed", "canceled"].includes(current.notification.status);
  const notification = normalizeIncidentNotification({
    ...current.notification,
    status: sent ? "sent" : closed ? "canceled" : "failed",
    sentAt: sent ? now : current.notification.sentAt,
    provider,
    providerMessageId: sent ? providerMessageId : null,
    deliveryClaimId: null,
    deliveryClaimedAt: null,
    providerSubmissionStartedAt: sent ? current.notification.providerSubmissionStartedAt : null,
    deliveryOutcomeUnknown: false,
    deliveryResolution: action,
    deliveryReconciledAt: now,
    deliveryReconciledBy: actorId,
    deliveryResolutionNote: resolutionNote,
    lastError: sent ? null : text(`Provider confirmed no delivery. ${resolutionNote}`, 1000)
  });
  const dispatch = normalizeIncidentDispatch({
    ...current,
    notification,
    ...
    timeline: appendDispatchTimeline(current, {
      at: now,
      action: sent ? "delivery-confirmed-sent" : "delivery-confirmed-not-sent",
      actorId,
      note: resolutionNote,
      fromStatus: current.status,
  ...
  const dispatches = doc.dispatches.slice();
  dispatches[index] = dispatch;
  return { ok: true, changed: true, action, dispatch, before: current, doc: { ...doc, lastUpdated: now, dispatches } };
}
...
export function summarizeIncidentDispatches(docInput) {
  ...
  return {
    ...
    sendingMessages: doc.dispatches.filter(item => item.notification.status === "sending").length,
    ...
    unknownDeliveryMessages: doc.dispatches.filter(item => item.notification.deliveryOutcomeUnknown).length
  };
}
...
function applyAutomatedIncidentSignal(docInput, signalInput, incidentInput, options = {}) {
  ...
  const signalMatchesIncidentSeverity = incidentLevelRank(signal.level) >= incidentLevelRank(active.severity);
  ...
  const incident = normalizeOperationsIncident({
    ...
    status: targetStatus,
    summary: signalMatchesIncidentSeverity ? incidentInput.summary : active.summary,
    latestSignalAt: signal.observedAt,
    latestLevel: signal.level,
    latestMetrics: signal.metrics,
