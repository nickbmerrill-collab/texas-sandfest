...
export function publicIslandConditions(docInput, now = new Date().toISOString()) {
  ...
  return {
    ...
    ferry: {
      status: summary.ferry.status,
      route: summary.ferry.route,
      source: summary.ferry.source,
      sourceUrl: summary.ferry.sourceUrl,
      observedAt: summary.ferry.observedAt,
      estimatedWaitMinutes: summary.ferry.estimatedWaitMinutes,
      operatingFerries: summary.ferry.operatingFerries,
      ...
      directions: Array.isArray(summary.ferry.directions) ? summary.ferry.directions.map(direction => ({
        ...
        estimatedWaitMinutes: direction.estimatedWaitMinutes ?? null,
...
export function normalizeNwsForecast(forecastPayload, alertsPayload, now = new Date().toISOString()) {
  ...
  const normalizedAlerts = alerts.slice(0, 10).map(feature => ({
    id: text(feature.id, 300), event: text(feature.properties?.event, 160),
    severity: text(feature.properties?.severity, 40), headline: text(feature.properties?.headline, 300),
    expiresAt: iso(feature.properties?.expires)
  }));
  if (!period) {
    return {
      status: "unavailable",
