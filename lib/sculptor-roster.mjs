...
export function sculptorRosterReadiness(input = {}, options = {}) {
  ...
  const sourceAgeDays = sourceCheckedAt
    ...
    : null;
  const currentSource = clockReady && sourceAgeDays !== null && sourceAgeDays >= 0 && sourceAgeDays <= maxSourceAgeDays;
  ...
  return {
    ...
    checks: {
      ...
      source: publication.mode === "demo" || currentSource
    },
    missing: [
      ...(!clockReady ? ["clock"] : []),
      ...(!publication.visible ? ["publication"] : []),
      ...(publication.visible && publication.mode !== "demo" && !currentSource ? ["source"] : [])
    ],
    sourceAgeDays,
    maxSourceAgeDays,
    engagement: {
