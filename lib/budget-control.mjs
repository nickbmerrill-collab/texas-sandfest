...
export function createBudgetLine(raw, input, { actorId = "unknown", idFactory, now = new Date().toISOString() } = {}) {
  ...
  const id = clean(idFactory?.("budget_line") || `budget_line_${Date.now()}`, 120);
  const existing = doc.budgetLines.find(line => line.id === id);
  if (existing) {
    const replay = existing.name === name
      && existing.ownerTeam === ownerTeam
      && existing.budgetCents === budgetCents
      && existing.notes === clean(input?.notes, 500)
      && existing.createdBy === actor;
    return replay
      ? { ok: true, replay: true, line: existing, doc }
      : failure("This budget request key was already used for different allocation details.", "IDEMPOTENCY_CONFLICT");
  }
...
export function createExpenseRequest(raw, input, { actorId = "unknown", idFactory, now = new Date().toISOString() } = {}) {
  ...
  const actor = clean(actorId, 180);
  const id = clean(idFactory?.("expense") || `expense_${Date.now()}`, 120);
  const existing = doc.expenses.find(expense => expense.id === id);
  if (existing) {
    const replay = existing.budgetLineId === line.id
    ...
    return replay
      ? { ok: true, replay: true, expense: existing, doc }
      : failure("This expense request key was already used for different submission details.", "IDEMPOTENCY_CONFLICT");
  }
  const at = instant(now, new Date().toISOString());
  const expense = {
    id,
  ...
  return { ok: true, replay: false, expense, doc: { ...doc, lastUpdated: at, expenses: [...doc.expenses, expense] } };
}
