...
export function visitorGuidanceEntryErrors(input = [], options = {}) {
  ...
  const errors = [];
  if (!clockReady) errors.push("Visitor guidance clock is invalid.");
  ...
  guidance.forEach((item, index) => {
    ...
    if (clockReady && item.sourceCheckedAt && item.sourceCheckedAt > now.toISOString()) errors.push(`${label} source-review time cannot be in the future.`);
...
export function visitorGuidanceReadiness(input = {}, options = {}) {
  ...
  const checks = {
    ...
    source: boardDemo || Boolean(clockReady && publication.sourceUrl && sourceCheckedAt && sourceAgeDays >= 0 && sourceAgeDays <= maxSourceAgeDays),
