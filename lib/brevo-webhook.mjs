...
export function applyBrevoDeliveryEvents(docInput, eventsInput = [], options = {}) {
  ...
  if (!Number.isFinite(nowMs)) return { ok: false, error: "Brevo webhook receipt time is invalid.", doc };
  ...
  for (const event of candidateById.values()) {
    ...
    if (reason && followup.prospectId) {
      ...
      if (prospect && !prospect.suppressedAt && prospect.status !== "do_not_contact") {
        const activeBefore = doc.followups.filter(item => item.prospectId === prospect.id && ["pending", "draft_ready", "approved", "queued", "sending", "failed"].includes(item.status)).length;
        ...
        if (result.ok) {
          doc = result.doc;
          suppressed += 1;
          const activeAfter = doc.followups.filter(item => item.prospectId === prospect.id && ["pending", "draft_ready", "approved", "queued", "sending", "failed"].includes(item.status)).length;
