...
} from "./visitor-guidance.mjs";

const PUBLIC_SCHEDULE_CATEGORIES = new Set(EVENT_SCHEDULE_CATEGORIES.map(item => item.toLowerCase()));
const ROOT_KEYS = new Set(["guide", "guidance", "schedule", "zones", "alert", "runtime"]);
...
const GUIDANCE_KEYS = new Set(["id", "category", "question", "answer", "keywords", "sourceLabel", "sourceUrl", "sourceCheckedAt", "effectiveAt", "expiresAt"]);
...
export function publicAppBootstrap(input = {}, options = {}) {
  ...
  const guidanceReadiness = visitorGuidanceReadiness({
    ...
    publication: source.guidancePublication
  }, {
    now: options.now,
    maxSourceAgeDays: options.maxGuidanceSourceAgeDays,
    allowBoardDemo: boardDemo
  });
  const output = {
    guide: publicEventGuide(source.guide),
    guidance: guidanceReadiness.ready ? publicVisitorGuidance(source.guidance, { now: options.now }) : [],
...
export function publicAppBootstrapSafety(input = {}, options = {}) {
  ...
  for (const [index, item] of (Array.isArray(input.guidance) ? input.guidance : []).entries()) {
    const unknown = unknownKeys(item, GUIDANCE_KEYS);
    if (unknown.length) errors.push(`Unexpected public guidance keys at ${index}: ${unknown.join(", ")}.`);
    if (!item?.id || !item?.question || !item?.answer || !item?.sourceLabel || !/^https:\/\//i.test(item?.sourceUrl || "")) {
      errors.push(`Public guidance entry ${index} is incomplete.`);
    }
    if (!Array.isArray(item?.keywords) || item.keywords.some(keyword => typeof keyword !== "string" || !keyword.trim())) {
      errors.push(`Public guidance entry ${index} has invalid search keywords.`);
    }
  }
