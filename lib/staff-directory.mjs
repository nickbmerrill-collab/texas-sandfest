...
export function staffDirectoryReadiness(input, options = {}) {
  ...
  const clockReady = Number.isFinite(nowMs);
  const stale = !Number.isFinite(verifiedMs) || (clockReady && nowMs - verifiedMs > maxAgeDays * 86_400_000);
  const errors = [];
  if (!clockReady) errors.push("staff directory clock is invalid");
