...
export async function verifyProductionApi({
  ...
  add("api.partner_recovery", "api", partnerIntake.response?.status === 200 && partnerIntakeSafety.ready && partnerIntake.json?.recoveryAvailable === true, partnerIntakeDetail || (partnerIntake.json?.recoveryAvailable === true ? "Partner private-access email is available." : "Partner private-access email reports unavailable."));
...
export async function verifyLiveDeployment({
  ...
  const [requests, apiVerification] = await Promise.all([
    ...
    verifyProductionApi({ config, fetchImpl, now })
  ]);
