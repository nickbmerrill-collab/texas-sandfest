...
import { publicGuestServicesReadinessSafety } from "./guest-services.mjs";
import { publicPartnerServerReadinessSafety } from "./public-partner-server-readiness.mjs";
import { publicAppBootstrapSafety } from "./public-bootstrap.mjs";
import { publicConciergeResponseSafety } from "./public-concierge.mjs";
import {
  sandfestAppleApplicationIdentifier,
  sandfestAppleAppSiteAssociationSafety
} from "./public-deep-links.mjs";
import { publicMediaManifestSafety } from "./public-media-manifest.mjs";

const DEFAULT_PUBLIC_URL = "https://sandfest.heyelab.com/";
const DEFAULT_API_URL = "https://sandfest-api.heyelab.com/";
...
export function deploymentVerificationConfig(env = process.env) {
  ...
  let appleApplicationIdentifier = null;
  try {
    appleApplicationIdentifier = sandfestAppleApplicationIdentifier(env.SANDFEST_APPLE_APP_ID_PREFIX);
  } catch (error) {
...
function exactAssetMatch(expectedHtml, actualHtml) {
  ...
  const actual = compiledAssetNames(actualHtml);
  const actualSet = new Set(actual);
  const expectedSet = new Set(expected);
  const missing = expected.filter(asset => !actualSet.has(asset));
  const unexpected = actual.filter(asset => !expectedSet.has(asset));
  return { ok: expected.length > 0 && missing.length === 0 && unexpected.length === 0, expected, actual, missing, unexpected };
}
...
function verificationResult(config, checks, now) {
...
export async function verifyProductionApi({
  config,
  ...
  if (!config?.ready) {
    ...
    return verificationResult(config || {}, checks, now);
  }

  const publicOrigin = new URL(config.publicUrl).origin;
  const adminOrigin = new URL(config.adminUrl).origin;
  const untrustedOrigin = "https://texassandfest.org.evil.example";
  const requests = await Promise.all([
    ...
    readJson(fetchImpl, targetUrl(config.apiUrl, "api/public/partner-intake"), {}, config.timeoutMs),
    readJson(fetchImpl, targetUrl(config.apiUrl, "api/public/guest-services"), {}, config.timeoutMs),
    readJson(fetchImpl, targetUrl(config.apiUrl, "api/public/island-conditions"), {}, config.timeoutMs),
    readJson(fetchImpl, targetUrl(config.apiUrl, "api/public/concierge"), {
      method: "POST",
      headers: { "content-type": "application/json", origin: publicOrigin },
      body: JSON.stringify({ question: "Where can I buy tickets?" })
    }, config.timeoutMs),
    ...
    readJson(fetchImpl, targetUrl(config.apiUrl, "api/public/bootstrap"), { headers: { origin: untrustedOrigin } }, config.timeoutMs)
  ]);
  const [health, ready, apiBootstrap, tickets, sponsors, vendors, partnerIntake, guestServices, island, concierge, publicCors, adminCors, untrustedCors] = requests;
  ...
  if (siteOnly) {
    add(
      "api.site_readiness_boundary",
      "api",
      [200, 503].includes(ready.response?.status)
        && ready.json
        && deployment.production === true
        && typeof deployment.ok === "boolean",
      ready.error || `HTTP ${ready.response?.status || "missing"}; launchReady=${deployment.ok}; errors=${deployment.errors ?? "missing"}. Provider gates may remain red for the board site-only lane.`
    );
  } else {
  ...
  const reportedCapabilities = Array.isArray(deployment.requiredCapabilities)
    ? deployment.requiredCapabilities.map(item => boundedText(item, 120)).filter(Boolean)
    : [];
  const activeCapabilities = new Set(reportedCapabilities);
  const requiredCapabilitySet = new Set(config.requiredCapabilities);
  const missingCapabilities = config.requiredCapabilities.filter(item => !activeCapabilities.has(item));
  const unexpectedCapabilities = reportedCapabilities.filter(item => !requiredCapabilitySet.has(item));
  add(
    "api.required_capabilities",
    "api",
    missingCapabilities.length === 0 && unexpectedCapabilities.length === 0,
    missingCapabilities.length
      ? `Missing required gates: ${missingCapabilities.join(", ")}.`
      : unexpectedCapabilities.length
        ? `Unexpected readiness gates: ${unexpectedCapabilities.join(", ")}.`
        : `${config.requiredCapabilities.length} production capability gates matched exactly.`
  );
  ...
  add("api.visitor_guidance", "api", apiBootstrap.response?.status === 200 && Array.isArray(apiBootstrap.json?.guidance) && apiBootstrap.json.guidance.length > 0, `${apiBootstrap.json?.guidance?.length || 0} current visitor answers published.`);
  const apiBootstrapSafety = publicAppBootstrapSafety(apiBootstrap.json || {});
  add("api.public_bootstrap_privacy", "api", apiBootstrap.response?.status === 200 && apiBootstrapSafety.ready, apiBootstrapSafety.ready ? "API bootstrap contains only approved visitor fields." : apiBootstrapSafety.errors.join(" "));
  const publicTicketProducts = Array.isArray(tickets.json?.products) ? tickets.json.products : [];
  const checkoutReadyProducts = publicTicketProducts.filter(item => item.availableForCheckout === true);
  add(
    siteOnly ? "api.ticket_catalog" : "api.ticket_checkout",
    "api",
    tickets.response?.status === 200
      && (siteOnly ? Array.isArray(tickets.json?.products) : checkoutReadyProducts.length > 0),
    siteOnly
      ? `${publicTicketProducts.length} public ticket products returned; checkout may remain gated until Stripe activation.`
      : `${publicTicketProducts.length} products; ${checkoutReadyProducts.length} checkout-ready.`
  );
  ...
  const partnerIntakeSafety = publicPartnerServerReadinessSafety(partnerIntake.json, { eventId: config.expectedEventId });
  const partnerIntakeDetail = partnerIntake.error
    || (!partnerIntakeSafety.ready ? partnerIntakeSafety.errors.join(" ") : null)
    || (partnerIntake.response?.status !== 200 ? responseDetail(partnerIntake.response) : null);
  add("api.partner_intake", "api", partnerIntake.response?.status === 200 && partnerIntakeSafety.ready && partnerIntake.json?.intakeAvailable === true, partnerIntakeDetail || (partnerIntake.json?.intakeAvailable === true ? "Partner intake can issue private access." : "Partner intake reports unavailable."));
  add(
    siteOnly ? "api.partner_recovery_boundary" : "api.partner_recovery",
    "api",
    partnerIntake.response?.status === 200
      && partnerIntakeSafety.ready
      && (siteOnly ? typeof partnerIntake.json?.recoveryAvailable === "boolean" : partnerIntake.json?.recoveryAvailable === true),
    partnerIntakeDetail || (partnerIntake.json?.recoveryAvailable === true
      ? "Partner private-access email is available."
      : siteOnly ? "Partner private-access email is safely reported as deferred or unavailable." : "Partner private-access email reports unavailable.")
  );
  add("api.partner_intake_privacy", "api", partnerIntake.response?.status === 200 && partnerIntakeSafety.ready, partnerIntakeSafety.ready ? "Partner readiness contains only approved public fields." : partnerIntakeSafety.errors.join(" "));
  const guestServicesSafety = publicGuestServicesReadinessSafety(guestServices.json, { eventId: config.expectedEventId });
  const guestServicesReady = guestServices.response?.status === 200 && guestServices.json?.available === true && guestServicesSafety.ready;
  const guestServicesDetail = guestServices.error
    || (!guestServicesSafety.ready ? guestServicesSafety.errors.join(" ") : null)
    || (guestServices.response?.status !== 200 ? responseDetail(guestServices.response) : null)
    || (guestServices.json?.available !== true ? "Guest Services intake reports unavailable." : null)
    || `${guestServices.json?.categories?.length || 0} Guest Services request categories are ready.`;
  add("api.guest_services_intake", "api", guestServicesReady, guestServicesDetail);
  add("api.guest_services_privacy", "api", guestServices.response?.status === 200 && guestServicesSafety.ready, guestServicesSafety.ready ? "Guest Services readiness exposes only approved public fields." : guestServices.error || guestServicesSafety.errors.join(" "));
  const conciergeSafety = publicConciergeResponseSafety(concierge.json || {});
  const conciergeReady = concierge.response?.status === 200
    && concierge.json?.topic === "tickets"
    && conciergeSafety.ready
    && concierge.response?.headers?.get?.("cache-control") === "no-store";
  add("api.public_concierge", "api", conciergeReady, conciergeReady ? "Ask Sandy returns source-cited, no-store public answers." : concierge.error || conciergeSafety.errors.join(" ") || responseDetail(concierge.response));
  ...
  if (siteOnly) {
    add(
      "api.live_conditions_boundary",
      "api",
      islandContractReady,
      `${liveCameraCount}/8 cameras live; weather=${islandBody.weather?.status || "missing"}, ferry=${islandBody.ferry?.status || "missing"}. Live weather, ferry, and camera freshness remain launch-provider gates.`
    );
  } else {
  ...
  add(
    "api.cors_rejects_untrusted_origin",
    "api",
    untrustedCors.response?.status === 200 && !corsOrigin(untrustedCors.response),
    `Untrusted origin ${untrustedOrigin} received ${corsOrigin(untrustedCors.response) || "no CORS grant"}.`
  );
...
export async function verifyLiveDeployment({
  ...
  const expectedAppleAssociation = artifacts?.publicAppleAssociation || null;
  const artifactAppleAssociationSafety = sandfestAppleAppSiteAssociationSafety(
    expectedAppleAssociation,
    config.appleApplicationIdentifier
  );
  ...
  add("artifacts.universal_links", "artifact", artifactAppleAssociationSafety.ready, artifactAppleAssociationSafety.ready ? "Public artifact binds approved HTTPS routes to the signed iOS app identity." : artifactAppleAssociationSafety.errors.join(" "));
  add("artifacts.admin", "artifact", compiledAssetNames(expectedAdminHtml).length > 0, "Admin artifact has compiled assets.");

  const appleAssociationUrl = new URL("/.well-known/apple-app-site-association", config.publicUrl).toString();
  const [requests, apiVerification] = await Promise.all([
    Promise.all([
      ...
      readJson(fetchImpl, targetUrl(config.publicUrl, "assets/sandfest-media/media-manifest.json"), {}, config.timeoutMs),
      readJson(fetchImpl, appleAssociationUrl, { redirect: "manual" }, config.timeoutMs),
      readText(fetchImpl, config.adminUrl, {}, config.timeoutMs)
    ]),
    verifyProductionApi({ config, fetchImpl, now, siteOnly })
  ]);
  const [publicPage, publicWorker, publicStaticBootstrap, publicMediaManifest, appleAssociation, adminPage] = requests;
  ...
  add(
    "public.artifact_freshness",
    "public",
    publicAssets.ok,
    publicAssets.missing.length
      ? `Missing current assets: ${publicAssets.missing.join(", ")}`
      : publicAssets.unexpected.length
        ? `Unexpected compiled assets are still referenced: ${publicAssets.unexpected.join(", ")}`
        : `${publicAssets.expected.length} current assets matched exactly.`
  );
  ...
  add("public.static_visitor_guidance", "public", publicStaticBootstrap.response?.status === 200 && Array.isArray(publicStaticBootstrap.json?.guidance) && publicStaticBootstrap.json.guidance.length > 0, `${publicStaticBootstrap.json?.guidance?.length || 0} current visitor answers available offline.`);
  const staticBootstrapSafety = publicAppBootstrapSafety(publicStaticBootstrap.json || {});
  add("public.static_bootstrap_privacy", "public", publicStaticBootstrap.response?.status === 200 && staticBootstrapSafety.ready, staticBootstrapSafety.ready ? "Static bootstrap contains only approved visitor fields." : staticBootstrapSafety.errors.join(" "));
  const mediaManifestSafety = publicMediaManifestSafety(publicMediaManifest.json || {});
  add("public.media_manifest_privacy", "public", publicMediaManifest.response?.status === 200 && mediaManifestSafety.ready, mediaManifestSafety.ready ? "Media manifest omits internal paths and source implementation fields." : mediaManifestSafety.errors.join(" "));
  const remoteAppleAssociationSafety = sandfestAppleAppSiteAssociationSafety(
    appleAssociation.json,
    config.appleApplicationIdentifier
  );
  const appleAssociationContentType = boundedText(appleAssociation.response?.headers?.get?.("content-type"), 200).toLowerCase();
  const appleAssociationDirect = appleAssociation.response?.status === 200
    && appleAssociation.response?.redirected !== true
    && !appleAssociation.response?.headers?.get?.("location")
    && (!appleAssociation.response?.url || appleAssociation.response.url === appleAssociationUrl);
  const appleAssociationMatchesArtifact = remoteAppleAssociationSafety.ready
    && JSON.stringify(appleAssociation.json) === JSON.stringify(expectedAppleAssociation);
  add(
    "public.universal_links",
    "public",
    appleAssociationDirect && appleAssociationContentType.includes("application/json") && appleAssociationMatchesArtifact,
    appleAssociation.error
      || (!appleAssociationDirect ? "AASA must return HTTP 200 from the exact .well-known URL without a redirect."
        : !appleAssociationContentType.includes("application/json") ? `AASA content type is ${appleAssociationContentType || "missing"}.`
          : remoteAppleAssociationSafety.errors.join(" ") || "Live AASA differs from the verified public artifact.")
  );
  ...
  add(
    "admin.artifact_freshness",
    "admin",
    adminAssets.ok,
    adminAssets.missing.length
      ? `Missing current assets: ${adminAssets.missing.join(", ")}`
      : adminAssets.unexpected.length
        ? `Unexpected compiled assets are still referenced: ${adminAssets.unexpected.join(", ")}`
        : `${adminAssets.expected.length} current assets matched exactly.`
  );
  ...
  checks.push(...apiVerification.checks);
