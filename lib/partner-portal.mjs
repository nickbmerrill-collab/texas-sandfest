...
export function vendorApplicationUrl(application, offering, options = {}) {
  const env = options.env ?? process.env;
  const config = options.config ?? partnerPortalConfig(env);
  const offeringId = clean(offering?.id, 100);
  const category = clean(application?.category, 40);
  if (!config.ready || !offeringId || !category) return null;
  const url = new URL(config.publicBaseUrl);
  if (!config.production && env.SANDFEST_API_PUBLIC_BASE_URL) {
    url.searchParams.set("apiBase", clean(env.SANDFEST_API_PUBLIC_BASE_URL, 1000));
  }
  url.searchParams.set("vendorOffering", offeringId);
  url.searchParams.set("vendorCategory", category);
  url.hash = "vendor-application-form";
  return url.toString();
}
...
export function publicPartnerPortalStatus(docInput, application, options = {}) {
  ...
  const canExposeLiveCheckout = Number.isFinite(nowMs);
  ...
  const paymentCheckouts = invoice
    ? (Array.isArray(docInput?.paymentCheckouts) ? docInput.paymentCheckouts : [])
      .filter(item => item.invoiceId === invoice.id && item.applicationId === application.id)
      .sort((a, b) => String(b.updatedAt || b.createdAt || "").localeCompare(String(a.updatedAt || a.createdAt || "")))
    : [];
  const paymentCheckout = paymentCheckouts.find(item => item.status === "open"
    && stripeHostedCheckoutUrl(item.checkoutUrl)
    && canExposeLiveCheckout
    && new Date(item.expiresAt || "").getTime() > nowMs) ?? null;
  const latestPaymentCheckout = paymentCheckouts[0] ?? null;
  let checkoutUnavailableReason = null;
  if (!isVendorInterest && balanceCents > 0 && !paymentCheckout) {
    const latestCheckoutExpired = canExposeLiveCheckout
      && Number.isFinite(new Date(latestPaymentCheckout?.expiresAt || "").getTime())
      && new Date(latestPaymentCheckout.expiresAt).getTime() <= nowMs;
    checkoutUnavailableReason = !canExposeLiveCheckout
      ? "clock_invalid"
      : !invoice
        ? "invoice_not_ready"
        : !["approved", "queued", "synced", "failed"].includes(invoice.status)
          ? "invoice_not_approved"
          : latestPaymentCheckout?.status === "creating"
            ? "checkout_preparing"
            : latestPaymentCheckout?.status === "open" && latestCheckoutExpired
              ? "checkout_expired"
              : latestPaymentCheckout?.status === "open" && !stripeHostedCheckoutUrl(latestPaymentCheckout.checkoutUrl)
                ? "checkout_invalid"
                : ["failed", "reconciliation_required"].includes(latestPaymentCheckout?.status)
                  ? "checkout_needs_retry"
  ...
  const vendorAssignment = (Array.isArray(docInput?.vendorAssignments) ? docInput.vendorAssignments : [])
    .find(item => item.applicationId === application.id) ?? null;
  const messages = (Array.isArray(docInput?.followups) ? docInput.followups : [])
    .filter(item => item.applicationId === application.id && item.status !== "dismissed")
    .sort((a, b) => String(b.sentAt || b.updatedAt || b.createdAt || "").localeCompare(String(a.sentAt || a.updatedAt || a.createdAt || "")))
    .slice(0, 5)
    .map(item => ({
      subject: clean(item.subject, 180) || clean(item.kind, 80).replace(/[-_]/g, " ") || "SandFest message",
      status: clean(item.deliveryStatus || item.status, 40) || "pending",
  ...
  return {
    ...
    intakeMode: isVendorInterest ? "interest" : "application",
    ...
    contactPreference: {
      allowed: application.consentToContact === true,
      version: Math.max(1, Math.round(Number(application.consentPreferenceVersion) || 1)),
      updatedAt: application.consentPreferenceUpdatedAt ?? application.consentCapturedAt ?? null
    },
    finance: {
      currency: "usd",
      expectedAmountCents,
      paidAmountCents,
      balanceCents,
      paymentStatus: isVendorInterest ? "not_applicable" : expectedAmountCents === 0 ? "pending_review" : balanceCents === 0 ? "paid" : paidAmountCents > 0 ? "partial" : "unpaid",
      invoice: invoice ? {
        id: invoice.id,
      ...
      checkout: paymentCheckout ? {
        status: "open",
        checkoutUrl: stripeHostedCheckoutUrl(paymentCheckout.checkoutUrl),
    ...
    vendorOnboarding: application.type === "vendor" && !isVendorInterest ? {
