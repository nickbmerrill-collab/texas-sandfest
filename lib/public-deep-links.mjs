...
const PUBLIC_WEB_HASHES = {
  today: "#top",
  tickets: "#tickets",
  "island-conditions": "#island-conditions",
  sculptors: "#sculptors-showcase"
};
...
function boundedQuestion(search) {
  const question = (new URLSearchParams(search).get("question") || "").trim();
...
export function canonicalPublicWebRoute(pathname, { basePath = "/", search = "" } = {}) {
  let path;
  let base;
  try {
    path = decodeURIComponent(pathname);
    base = decodeURIComponent(basePath).replace(/\/+$/, "");
  } catch {
    return null;
  }
  if (base && !path.startsWith(`${base}/`)) return null;
  const components = path.slice(base.length).split("/").filter(Boolean);
  if (!components.length) return null;

  const destination = components[0].toLowerCase();
  const publicHash = PUBLIC_WEB_HASHES[destination];
  if (publicHash) {
    return components.length === 1 ? { hash: publicHash } : null;
  }
  switch (destination) {
  case "sandy":
    return components.length === 1
      ? { hash: "#concierge", question: boundedQuestion(search) }
      : null;
  case "schedule": {
    if (components.length === 1) return { hash: "#schedule" };
