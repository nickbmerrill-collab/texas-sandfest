...
export function createSmsAlertCampaign(raw, { alert, recipients = [], limit = 500 } = {}, opts = {}) {
  ...
  if (!now) return { ok: false, error: "SMS operation time is invalid.", doc, campaign: null, messages: [] };
...
export function attachSmsJobs(raw, assignments = [], opts = {}) {
  const now = nowIso(opts.now);
  const doc = normalizeSmsOperations(raw, { eventId: opts.eventId });
  if (!now) return { ok: false, error: "SMS operation time is invalid.", doc, messages: [] };
...
export function beginSmsSubmission(raw, messageId, opts = {}) {
  const now = nowIso(opts.now);
  const doc = normalizeSmsOperations(raw, { eventId: opts.eventId });
  if (!now) return { ok: false, error: "SMS operation time is invalid.", doc, message: null };
...
export function recordSmsSubmission(raw, messageId, result = {}, opts = {}) {
  ...
  if (index === -1) return { ok: false, error: "SMS message record not found.", doc };
  const current = doc.messages[index];
  if (current.providerMessageSid && result.sid && current.providerMessageSid !== result.sid) {
    return { ok: false, error: "Twilio MessageSid did not match the stored SMS message.", doc, message: current };
  }
  ...
  const providerLifecycleWon = Boolean(current.providerStatus)
    && (TERMINAL_STATUSES.has(current.status) || currentRank > nextRank);
  doc.messages[index] = {
    ...current,
    status: providerLifecycleWon ? current.status : status,
    providerStatus: providerLifecycleWon ? current.providerStatus : result.status || null,
    providerMessageSid: result.sid || current.providerMessageSid || null,
    errorCode: providerLifecycleWon ? current.errorCode : result.twilioCode == null ? null : String(result.twilioCode),
    error: providerLifecycleWon ? current.error : result.error ? String(result.error).slice(0, 500) : null,
    submittedAt: result.ok ? current.submittedAt || now : current.submittedAt,
    terminalAt: providerLifecycleWon ? current.terminalAt : TERMINAL_STATUSES.has(status) ? now : null,
    updatedAt: now
  };
  doc.lastUpdated = now;
  refreshCampaigns(doc, now, [doc.messages[index].campaignId]);
  return { ok: true, ignoredRegression: providerLifecycleWon, doc, message: doc.messages[index] };
}
...
export function recordSmsPreferenceEvent(raw, input = {}, opts = {}) {
  const now = nowIso(opts.now);
  const doc = normalizeSmsOperations(raw, { eventId: opts.eventId });
  if (!now) return { ok: false, error: "SMS operation time is invalid.", doc, event: null };
...
export function suppressSmsCampaignsForAlert(raw, alertId, opts = {}) {
  const now = nowIso(opts.now);
  const doc = normalizeSmsOperations(raw, { eventId: opts.eventId });
  if (!now) return { ok: false, error: "SMS operation time is invalid.", suppressed: 0, doc };
