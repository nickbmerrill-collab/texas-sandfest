...
export function evaluateBoardDemoReadiness(state = {}) {
  ...
  const islandFeedsReady = weather.status === "live"
    ...
    && ferryCurrent;
  const islandFeedsDetail = ferry.status === "live"
  ...
  const checks = [
    ...
    check("operations", "Seeded operations", operationsReady, operationsReady ? `${applicationSummary.total} applications, all ${staffDirectory.totalTeams} team routes, the extracted briefing, and published sponsor branding are ready.` : "The board runtime is missing partner records, staff routes, local email readiness, the extracted briefing, or published sponsor branding.", "Rebuild with npm run board:runtime and restart the API."),
