...
export const BOARD_DEMO_STACK_CHECK_COUNT = 11;
export const BOARD_DEMO_PREFLIGHT_CHECK_COUNT = BOARD_DEMO_STACK_CHECK_COUNT + 1;
...
export function evaluateBoardDemoReadiness(state = {}) {
  ...
  const visitorGuidance = Array.isArray(state.bootstrap?.guidance) ? state.bootstrap.guidance : [];
  const visitorGuidanceReady = visitorGuidance.length >= 6
    && visitorGuidance.every(item => item?.id && item?.question && item?.answer && /^https:\/\//.test(String(item?.sourceUrl || "")));
  const sculptors = Array.isArray(state.sculptors?.sculptors) ? state.sculptors.sculptors : [];
  const sculptureEntries = Array.isArray(state.sculptors?.entries) ? state.sculptors.entries : [];
  const sculpturePois = Array.isArray(state.sculptors?.pois) ? state.sculptors.pois : [];
  const sculptorIds = new Set(sculptors.map(item => item?.id).filter(Boolean));
  const entryIds = new Set(sculptureEntries.map(item => item?.id).filter(Boolean));
  const sculpturePoiEntries = new Set(sculpturePois
    .filter(item => item?.type === "sculpture" && item?.entryId)
    .map(item => item.entryId));
  const sculptorRosterReady = state.sculptors?.meta?.eventId === state.bootstrap?.guide?.id
    && state.sculptors?.meta?.publicationStatus === "sample"
    && sculptors.length === 6
    && sculptureEntries.length === 6
    && sculptorIds.size === sculptors.length
    && entryIds.size === sculptureEntries.length
    && sculptors.every(item => entryIds.has(item?.entryId))
    && sculptureEntries.every(item => sculptorIds.has(item?.sculptorId) && sculpturePoiEntries.has(item?.id));
  ...
  const preparedTaskAssignmentMessages = taskAssignmentMessages.filter(item => (
    String(item?.body || "").includes("#task-status?task=") && String(item.body).includes("&token=tsft_")
  ));
  const taskAssignmentPortalsReady = preparedTaskAssignmentMessages.length >= 2;
  const deliveredLocalMessages = followups.filter(hasDurableBoardEmailDelivery);
  const providerVerificationMessages = followups.filter(item => item?.status === "delivery_unknown" && item?.deliveryOutcomeUnknown === true);
  ...
  const attendance = state.volunteers?.attendance || {};
  const attendanceReady = state.volunteers?.eventId === state.bootstrap?.guide?.id
    && Number(attendance.summary?.assigned || 0) >= 10
    && Number(attendance.summary?.checkedIn || 0) >= 1
    && Array.isArray(attendance.assignments)
    && attendance.assignments.some(item => item?.attendanceStatus === "checked_in" && item?.canCheckOut === true);
  ...
  const checks = [
    ...
    check("visitor_guidance", "Visitor guidance", visitorGuidanceReady, visitorGuidanceReady ? `${visitorGuidance.length} reviewed Plan Your Visit answers are available to the site and Ask Sandy.` : "The board runtime is missing reviewed visitor guidance.", "Reset the board runtime and confirm the public bootstrap publishes current guidance."),
    check("sculptor_roster", "Sculptor roster", sculptorRosterReady, sculptorRosterReady ? "Six linked synthetic sculptors, entries, and map points power the roster, passport, and ballot." : "The board runtime is missing its governed synthetic sculptor publication or contains broken roster linkage.", "Stop the stack, rebuild it with npm run board:demo -- --reset, and rerun npm run board:check."),
    ...
    check("operations", "Seeded workflows", operationsReady, operationsReady ? `${applicationSummary.total} applications, ${partners.invoices.length} invoice, ${partners.payments.length} payment, local ticket and partner-invoice checkout, ${budgetCounts.budgetLines} budget allocations, ${budgetCounts.expenses} expense approvals with accounting downloads, ${checkoutProducts.length} local-checkout ticket products, ${milestones.length} key dates, staff/volunteer/team delegation with live shift attendance, ${localAutomationReady ? `${deliveredLocalMessages.length} locally delivered messages including vendor opening, payment confirmation, and key-date follow-up${providerVerificationMessages.length ? `, plus ${providerVerificationMessages.length} provider verification item${providerVerificationMessages.length === 1 ? "" : "s"} locked for staff review` : ""}` : `${followups.length} review-ready message records`}, ready and blocked vendor paths, sponsor fulfillment, and targeted outreach are ready.` : "The board runtime is missing budget approvals or accounting downloads, finance, local payment checkout, vendor opening notice, payment confirmation, due-soon key dates, automatic key-date follow-up, local messaging proof, staff/volunteer/team delegation, live volunteer attendance, sponsor fulfillment, vendor readiness, outreach, governed documents, staff routes, or published sponsor branding.", "Stop the stack, rebuild it with npm run board:demo -- --reset, and rerun npm run board:check."),
  ...
  if (checks.length !== BOARD_DEMO_STACK_CHECK_COUNT) {
    throw new Error(`Board readiness contract expected ${BOARD_DEMO_STACK_CHECK_COUNT} checks but produced ${checks.length}.`);
  }
