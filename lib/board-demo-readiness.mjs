...
export function evaluateBoardDemoReadiness(state = {}) {
  ...
  const messagingReady = partners.email?.ready === true
    ...
    && (partners.automationMode === "review_first" || partners.automation?.active === true)
  ...
  const checks = [
    ...
    check("operations", "Seeded workflows", operationsReady, operationsReady ? `${applicationSummary.total} applications, ${partners.invoices.length} invoice, ${partners.payments.length} payment, ${milestones.length} key dates, staff/volunteer/team delegation, ${followups.length} message records, ready and blocked vendor paths, sponsor fulfillment, and targeted outreach are ready.` : "The board runtime is missing finance, key dates, messaging, staff/volunteer/team delegation, sponsor fulfillment, vendor readiness, outreach, governed documents, staff routes, or published sponsor branding.", "Stop the stack, rebuild it with npm run board:demo -- --reset, and rerun npm run board:check."),
