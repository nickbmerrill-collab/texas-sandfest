...
export function evaluateBoardDemoReadiness(state = {}) {
  ...
  const deliveredLocalMessages = followups.filter(item => item?.status === "sent" && item?.deliveryStatus === "delivered");
  ...
  const keyDatesReady = milestones.length >= 8 && milestones.every(item => item?.applicationId && Number.isFinite(Date.parse(item?.dueAt)));
  ...
  const localAutomationReady = partners.automationMode === "transactional_auto"
    ...
    && Number(state.emailSandbox?.acceptedMessages || 0) >= 2
    && Number(state.emailSandbox?.deliveryCallbacks || 0) >= 2
  ...
  const messagingReady = partners.email?.ready === true
    ...
    && acknowledgmentMessages.length >= 4
    && taskAssignmentMessages.length >= 2;
  ...
  const checks = [
    ...
    check("operations", "Seeded workflows", operationsReady, operationsReady ? `${applicationSummary.total} applications, ${partners.invoices.length} invoice, ${partners.payments.length} payment, ${milestones.length} key dates, staff/volunteer/team delegation, ${localAutomationReady ? `${deliveredLocalMessages.length} locally delivered messages` : `${followups.length} review-ready message records`}, ready and blocked vendor paths, sponsor fulfillment, and targeted outreach are ready.` : "The board runtime is missing finance, key dates, local messaging proof, staff/volunteer/team delegation, sponsor fulfillment, vendor readiness, outreach, governed documents, staff routes, or published sponsor branding.", "Stop the stack, rebuild it with npm run board:demo -- --reset, and rerun npm run board:check."),
