...
export function evaluateBoardDemoReadiness(state = {}) {
  ...
  const checks = [
    ...
    check("operations", "Seeded workflows", operationsReady, operationsReady ? `${applicationSummary.total} applications, ${partners.invoices.length} invoice, ${partners.payments.length} payment, local ticket and partner-invoice checkout, ${budgetCounts.budgetLines} budget allocations, ${budgetCounts.expenses} expense approvals, ${checkoutProducts.length} local-checkout ticket products, ${milestones.length} key dates, staff/volunteer/team delegation, ${localAutomationReady ? `${deliveredLocalMessages.length} locally delivered messages including vendor opening, payment confirmation, and key-date follow-up` : `${followups.length} review-ready message records`}, ready and blocked vendor paths, sponsor fulfillment, and targeted outreach are ready.` : "The board runtime is missing budget approvals, finance, local payment checkout, vendor opening notice, payment confirmation, due-soon key dates, automatic key-date follow-up, local messaging proof, staff/volunteer/team delegation, sponsor fulfillment, vendor readiness, outreach, governed documents, staff routes, or published sponsor branding.", "Stop the stack, rebuild it with npm run board:demo -- --reset, and rerun npm run board:check."),
