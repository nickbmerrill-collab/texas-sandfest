...
const QUEUE_JOB_ID = /^job_[a-zA-Z0-9-]{8,80}$/;
...
export async function markTerminalJobHandled(root, jobId, { now = Date.now() } = {}) {
  await assertRuntimeOwnership(root);
  if (!QUEUE_JOB_ID.test(String(jobId || ""))) return { ok: false, reason: "invalid_job_id" };
  ...
  if (usePostgres()) {
    ...
    return result.rowCount === 1
      ? {
          ok: true,
          handledAt,
          job: { ...rowToJob(result.rows[0]), failureHandledAt: handledAt }
        }
      : { ok: false, reason: "not_unhandled_failure" };
  }
  return withRuntimeOwnership(root, async () => {
    ...
    await writeFile(file, `${JSON.stringify(job, null, 2)}\n`);
    return { ok: true, handledAt, job: publicJob(job) };
  });
}
