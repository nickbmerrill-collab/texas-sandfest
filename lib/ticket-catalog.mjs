...
export function ticketCheckoutPolicyReadiness(catalogInput = {}, options = {}) {
  ...
  if (!clockReady) errors.push("Ticket checkout policy clock is invalid.");
  ...
  if (clockReady && approvedAt && Number.isFinite(approvedAt.getTime()) && approvedAt.getTime() > now.getTime() + 5 * 60_000) errors.push("Ticket checkout policy approval cannot be in the future.");
...
export function publicTicketCatalog(catalogInput, options = {}) {
  ...
  const policy = ticketCheckoutPolicyReadiness(catalog, { eventId: options.eventId, now: options.now });
