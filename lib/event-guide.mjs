...
export const VOLUNTEER_REGISTRATION_STATUSES = ["upcoming", "open", "closed", "paused"];
...
function normalizeVolunteerProgram(input = {}) {
  const registrationStatusInput = text(input.registrationStatus, 20).toLowerCase();
  const registrationStatus = VOLUNTEER_REGISTRATION_STATUSES.includes(registrationStatusInput)
    ? registrationStatusInput
    : "upcoming";
  return {
    informationUrl: validHttpsUrl(input.informationUrl),
    registrationStatus,
    registrationUrl: validHttpsUrl(input.registrationUrl),
    sourceCheckedAt: validInstant(input.sourceCheckedAt),
    note: text(input.note, 500)
  };
}

export function publicVolunteerProgram(input = {}) {
  const program = normalizeVolunteerProgram(input);
  return {
    informationUrl: program.informationUrl,
    registrationStatus: program.registrationStatus,
    registrationUrl: program.registrationStatus === "open" ? program.registrationUrl : null,
    sourceCheckedAt: program.sourceCheckedAt,
    note: program.note
  };
}

export function volunteerProgramReadiness(input = {}, options = {}) {
  const program = normalizeVolunteerProgram(input);
  const now = new Date(options.now ?? Date.now());
  const clockReady = Number.isFinite(now.getTime());
  const maxSourceAgeDays = Math.max(1, Number(options.maxSourceAgeDays ?? 90));
  const sourceCheckedAt = program.sourceCheckedAt ? new Date(program.sourceCheckedAt) : null;
  const sourceAgeDays = sourceCheckedAt
    ? Math.floor((now.getTime() - sourceCheckedAt.getTime()) / 86_400_000)
    : null;
  const checks = {
    clock: clockReady,
    information: Boolean(program.informationUrl),
    registration: program.registrationStatus !== "open" || Boolean(program.registrationUrl),
    source: Boolean(clockReady && sourceCheckedAt && sourceAgeDays >= 0 && sourceAgeDays <= maxSourceAgeDays)
  };
  ...
  return {
    ...
    sourceAgeDays,
    maxSourceAgeDays,
    program: publicVolunteerProgram(program),
    reason: missing.length
      ? `Volunteer program is not publish-ready: ${missing.join(", ")}.`
      : program.registrationStatus === "open"
        ? `Volunteer registration is open from a source reviewed ${sourceAgeDays} day${sourceAgeDays === 1 ? "" : "s"} ago.`
        : `Volunteer information is current; registration is ${program.registrationStatus}.`
  };
}
...
export function normalizeEventGuide(input = {}) {
  ...
  return {
    ...
    sourceCheckedAt: validInstant(input.sourceCheckedAt),
    volunteer: normalizeVolunteerProgram(input.volunteer),
...
export function publishEventGuide(currentInput, patch = {}, options = {}) {
  ...
  if (!candidate.volunteer.informationUrl) errors.push("An HTTPS official volunteer information URL is required.");
  if (!candidate.volunteer.sourceCheckedAt) errors.push("The volunteer source check time is required.");
  if (candidate.volunteer.sourceCheckedAt && now && candidate.volunteer.sourceCheckedAt > now) errors.push("The volunteer source check time cannot be in the future.");
  if (candidate.volunteer.registrationStatus === "open" && !candidate.volunteer.registrationUrl) {
    errors.push("An HTTPS volunteer registration URL is required while registration is open.");
  }
...
export function publicEventGuide(input = {}) {
  ...
  return {
    ...
    volunteer: publicVolunteerProgram(guide.volunteer),
...
export function eventGuideReadiness(input = {}, options = {}) {
  ...
  const volunteer = volunteerProgramReadiness(guide.volunteer, { now, maxSourceAgeDays });
  const checks = {
    clock: clockReady,
    ...
    upcoming: Boolean(clockReady && endAt && endAt.getTime() >= now.getTime()),
    ...
    source: Boolean(clockReady && guide.sourceUrl && sourceCheckedAt && sourceAgeDays >= 0 && sourceAgeDays <= maxSourceAgeDays),
  ...
  return {
    ...
    missing,
    sourceAgeDays,
    volunteerSourceAgeDays: volunteer.sourceAgeDays,
    maxSourceAgeDays,
