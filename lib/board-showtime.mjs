...
export function assessBoardBriefingDeck({
  ...
  const slideText = slides.map(entry => presentationXmlText(slideXml[entry] || "")).join("\n");
  ...
  for (const phrase of [
    ...
    if (!slideText.includes(phrase)) errors.push(`The board briefing does not mention "${phrase}".`);
  }
  return {
    ok: errors.length === 0,
