...
function requestTimeoutMs(value) {
  return boundedInteger(value, 1_000, 30_000, 5_000);
}

function boardFetch(url, options = {}, init = {}) {
  return (options.fetchImpl ?? fetch)(url, {
    ...init,
    signal: init.signal ?? AbortSignal.timeout(requestTimeoutMs(options.timeoutMs))
  });
}
...
async function responseJson(response, label) {
  ...
  if (!response.ok) {
    const error = new Error(`${label} failed (${response.status}): ${data.error || "unexpected response"}`);
...
async function adminRequest(apiBase, adminToken, pathname, options = {}) {
  const response = await boardFetch(`${apiBase}${pathname}`, options, {
...
async function signedCameraRequest(apiBase, secret, cameraId, endpoint, payload, options = {}) {
  ...
  const response = await boardFetch(`${apiBase}/api/ingest/cameras/${encodeURIComponent(cameraId)}/${endpoint}`, options, {
    method: "POST",
    headers: {
...
export async function verifyBoardCameraPlaybackTarget(options = {}) {
  ...
  const response = await boardFetch(`${apiBase}/api/public/bootstrap`, options, { cache: "no-store" });
...
export async function runBoardCameraPlaybackTick(options = {}) {
  ...
  if (options.verifyTarget !== false) await verifyBoardCameraPlaybackTarget({ apiBase, fetchImpl: options.fetchImpl, timeoutMs: options.timeoutMs });

  const cycle = boundedInteger(options.cycle, 0, Number.MAX_SAFE_INTEGER, 0);
  const runId = cleanId(options.runId, "board");
  const observedAt = new Date(options.observedAt || Date.now()).toISOString();
  const admin = await adminRequest(apiBase, adminToken, "/api/admin/island-conditions", { fetchImpl: options.fetchImpl, timeoutMs: options.timeoutMs });
  ...
  if (options.heartbeat !== false) {
    for (const profile of BOARD_CAMERA_PROFILES) {
      ...
      }), { fetchImpl: options.fetchImpl, timeoutMs: options.timeoutMs, nowMs: new Date(observedAt).getTime() });
      heartbeats += 1;
    }
  }
...
export function retryableBoardCameraPlaybackError(error) {
  if (typeof error?.retryable === "boolean") return error.retryable;
  if (["AbortError", "TimeoutError"].includes(error?.name)) return true;
  const causeCode = String(error?.cause?.code || error?.code || "");
  if (/^(ECONNREFUSED|ECONNRESET|EHOSTUNREACH|ENETUNREACH|ETIMEDOUT|UND_ERR_)/.test(causeCode)) return true;
  return error instanceof TypeError && /fetch|network|socket|terminated/i.test(String(error.message || ""));
}

export function boardCameraPlaybackRetryDelay(failureCount, options = {}) {
  const baseMs = boundedInteger(options.baseMs, 250, 30_000, 1_000);
  const maxMs = boundedInteger(options.maxMs, baseMs, 60_000, 30_000);
  const exponent = Math.max(0, Math.min(16, Math.round(Number(failureCount) || 1) - 1));
  return Math.min(maxMs, baseMs * 2 ** exponent);
}

export async function runBoardCameraPlayback(options = {}) {
  const intervalMs = boundedInteger(options.intervalMs, 250, 60_000, 5_000);
  const once = options.once === true;
  const sleepImpl = options.sleepImpl ?? (ms => new Promise(resolve => setTimeout(resolve, ms)));
  const verifyTargetImpl = options.verifyTargetImpl ?? verifyBoardCameraPlaybackTarget;
  const tickImpl = options.tickImpl ?? runBoardCameraPlaybackTick;
  ...
  while (!shouldStop()) {
    try {
      if (!target) {
        target = await verifyTargetImpl({
          apiBase: options.apiBase,
          fetchImpl: options.fetchImpl,
          timeoutMs: options.timeoutMs
        });
        await options.onVerified?.({ target, recovered: successfulTicks > 0 || consecutiveFailures > 0 });
      }
      const result = await tickImpl({
        ...
        cycle,
        configure,
        heartbeat: cycle % Math.max(1, Math.round(15_000 / intervalMs)) === 0,
      ...
      if (once || !retryableBoardCameraPlaybackError(error)) throw error;
      consecutiveFailures += 1;
      const delayMs = boardCameraPlaybackRetryDelay(consecutiveFailures, {
        baseMs: options.retryBaseMs,
        maxMs: options.retryMaxMs
      });
      target = null;
      configure = true;
      await options.onRetry?.({ error, delayMs, consecutiveFailures, cycle });
      await sleepImpl(delayMs);
    }
  }

  return { ok: true, cycle, successfulTicks, stopped: shouldStop(), once };
}
