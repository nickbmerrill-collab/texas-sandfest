...
const TICKET_REVENUE_STATUSES = new Set([
  "paid",
  "partially_refunded",
  "refunded"
]);
...
export function ticketRevenueEntries(orderEnvelopes = [], { eventId = null } = {}) {
  const entries = [];
  for (const envelope of Array.isArray(orderEnvelopes) ? orderEnvelopes : []) {
    const order = envelope?.record || envelope;
    if (!order || !TICKET_REVENUE_STATUSES.has(order.status)) continue;
    if (eventId && order.eventId !== eventId) continue;
    const amountCents = Math.max(0, toCents(order.totals?.knownAmount));
    if (!amountCents) continue;
    const quantity = (Array.isArray(order.lineItems) ? order.lineItems : [])
      .reduce((sum, line) => sum + Math.max(0, toCents(line.quantity)), 0);
    const externalRef = text(order.paymentIntentId || order.stripeCheckoutSessionId || order.id);
    const source = order.checkoutEnvironment === "board_sandbox"
      ? "manual"
      : order.provider === "stripe" ? "stripe" : "manual";
    const base = {
      ...
      category: "ticket",
      grossCents: amountCents,
      feeCents: Math.max(0, toCents(order.providerFeeCents)),
      netCents: amountCents - Math.max(0, toCents(order.providerFeeCents)),
      quantity,
      payoutId: order.payoutId ?? null,
      payoutDate: order.payoutDate ?? null,
      reconciled: Boolean(order.payoutId && order.payoutDate),
      qbClass: "Ticket Sales",
      qbAccount: "Ticket Revenue",
      externalRef,
      note: `${(order.lineItems || []).map(line => `${line.quantity} x ${line.name}`).join(", ") || "Ticket order"} (${order.id})`,
      entryType: "receipt",
      origin: order.checkoutEnvironment === "board_sandbox" ? "board_ticket_sandbox" : "ticket_orders",
      sourceRecordId: order.id,
      paymentStatus: order.status
    };
    entries.push(base);

    const refundedAmountCents = Math.min(amountCents, Math.max(0, toCents(order.refundedAmountCents)));
    if (!refundedAmountCents) continue;
    const fullyRefunded = refundedAmountCents === amountCents;
    entries.push({
      ...base,
      id: `ticket-order-refund:${order.id}`,
      date: order.refundedAt || order.updatedAt || base.date,
      grossCents: -refundedAmountCents,
      feeCents: 0,
      netCents: -refundedAmountCents,
      quantity: fullyRefunded ? -quantity : null,
      payoutId: null,
      payoutDate: null,
      reconciled: false,
      externalRef: `${externalRef}:refund`,
      note: `${fullyRefunded ? "Full" : "Partial"} ticket refund (${order.id})`,
      entryType: "refund"
    });
  }
  return entries;
...
export function buildRevenueLedgerView(ledger = {}, partnerOperations = {}, { eventId = null, ticketOrders = [] } = {}) {
  ...
  const tickets = ticketRevenueEntries(ticketOrders, { eventId });
  const entries = [];
  const seen = new Set();
  let duplicateEntries = 0;
  for (const entry of [...imported, ...partner, ...tickets]) {
  ...
  const ticketTimestamps = ticketOrders
    .map(item => item?.record || item)
    .map(order => order?.updatedAt)
    .filter(Boolean);
  const timestamps = [ledger.lastUpdated, partnerOperations.lastUpdated, ...ticketTimestamps]
  ...
  return {
    ...
    sources: {
      ...
      ticketOrders: {
        ...
        lastUpdated: ticketTimestamps.sort((a, b) => new Date(b).getTime() - new Date(a).getTime())[0] ?? null
      },
      duplicateEntries
    }
  };
