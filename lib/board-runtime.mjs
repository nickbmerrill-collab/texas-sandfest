...
function partnerDemo(eventId, now) {
  ...
  for (const followupId of [sponsor.followup.id, vendor.followup.id]) {
  ...
  return {
    ...doc,
    _note: "Synthetic 2027 board demonstration data. No external messages or payments are sent.",
    lastUpdated: now,
...
function engagementDemo(documents, eventId, now) {
  const huntId = documents.passportHunt.hunt.id;
  const checkpoints = documents.passportHunt.checkpoints || [];
  documents.passportHunt = {
    ...documents.passportHunt,
    hunt: {
      ...documents.passportHunt.hunt,
      active: true,
  ...
  documents.passportCompletions = {
    ...
    completions: checkpoints.slice(0, 4).map((checkpoint, index) => ({
      ...
      at: now,
      method: "qr_scan",
      pointsAwarded: checkpoint.points || 10
    }))
  };
  documents.voting = {
    ...documents.voting,
    votingOpen: true,
    votes: (documents.voting.entries || []).slice(0, 4).map((entry, index) => ({
...
export async function prepareBoardRuntime({ sourceRoot, targetRoot, eventId, now = new Date().toISOString(), replace = false } = {}) {
  ...
  engagementDemo(documents, targetEventId, now);
