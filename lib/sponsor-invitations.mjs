...
export function verifySponsorInvitationToken(docInput, token, options = {}) {
  ...
  const nowMs = now instanceof Date ? now.getTime() : Number.isFinite(Number(now)) ? Number(now) : new Date(now).getTime();
  if (!prospect || payload?.version !== 1 || !invitation) return { ok: false, error: "Sponsor invitation is invalid or has been revoked." };
  if (!Number.isFinite(nowMs)) return { ok: false, error: "Sponsor invitation verification time is invalid." };
  if (!Number.isFinite(expiresAt) || expiresAt <= nowMs) return { ok: false, expired: true, error: "Sponsor invitation expired. Ask the SandFest team for a new link." };
  if (
