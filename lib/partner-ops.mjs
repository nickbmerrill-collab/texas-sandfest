...
export function normalizePartnerOperations(input) {
  ...
  return {
    ...
    campaigns: Array.isArray(base.campaigns) ? base.campaigns : [],
...
export function updateOutreachCampaignStatus(docInput, campaignId, action, options = {}) {
  ...
  return {
    ...
    doc: {
      ...
      lastUpdated: now,
      campaigns,
      activity: [...doc.activity, activity(idFactory, `outreach.campaign.${action}`, "campaign", campaignId, now, options.actorId ?? "admin")].slice(-1000)
    }
  };
}
...
export function automatedFollowupQueueCandidates(docInput) {
  const doc = normalizePartnerOperations(docInput);
  if (doc.automationMode !== "transactional_auto") return [];
  return doc.followups.filter(item => item.status === "approved" && automatedApproval(item));
}
