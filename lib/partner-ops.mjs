import { stripeHostedCheckoutUrl } from "./stripe-checkout-url.mjs";

const APPLICATION_TYPES = new Set(["vendor", "sponsor"]);
const PARTNER_INTAKE_MODES = new Set(["interest", "application"]);
const APPLICATION_STATUSES = new Set([
  "submitted", "under_review", "approved", "contracted", "invoiced",
  "partial", "paid", "active", "complete", "rejected", "withdrawn"
]);
const APPLICATION_APPROVAL_STATUSES = new Set(["approved", "contracted", "invoiced", "partial", "paid", "active", "complete"]);
const APPLICATION_DECISIONS = new Set(["approved", "rejected"]);
...
const TASK_ASSIGNEE_ACTIONS = new Set(["acknowledge", "start", "block", "complete"]);
...
const FOLLOWUP_STATUSES = new Set(["pending", "draft_ready", "approved", "queued", "sending", "sent", "dismissed", "failed", "delivery_unknown"]);
const FOLLOWUP_DELIVERY_RESOLUTIONS = new Set(["confirmed_sent", "confirmed_not_sent"]);
...
export const PARTNER_TRANSACTIONAL_FOLLOWUP_KINDS = Object.freeze([
  ...
  "sponsor_brand_changes",
  "sponsor_deliverable_review",
...
export const PARTNER_PORTAL_RECOVERY_WINDOW_MS = 15 * 60 * 1000;
export const PARTNER_INITIAL_REMINDER_GRACE_MS = 24 * 60 * 60 * 1000;
const PARTNER_TRANSACTIONAL_FOLLOWUP_KIND_SET = new Set(PARTNER_TRANSACTIONAL_FOLLOWUP_KINDS);
const INITIAL_INTAKE_MILESTONE_KINDS = new Set(["interest_review", "application_review", "opportunity_qualification"]);
...
const OUTREACH_CAMPAIGN_EXCLUSION_LABELS = Object.freeze({
  suppressed: "Suppressed",
  status: "Not outreach-ready",
  missing_email: "Missing business email",
  missing_contact_basis: "Missing contact basis",
  below_fit: "Below minimum fit",
  industry: "Outside selected industries",
  city: "Outside selected cities",
  state: "Outside selected states",
  postal_code: "Outside selected ZIP codes",
  missing_location: "Missing coordinates",
  outside_radius: "Outside campaign radius"
});
const SANDFEST_LOCATION = { latitude: 27.8339, longitude: -97.0611 };
const EARTH_RADIUS_MILES = 3958.8;
const APPLICATION_IDENTITY_MAX_ATTEMPTS = 100;
...
function money(amountCents) {
  return new Intl.NumberFormat("en-US", { style: "currency", currency: "USD" }).format(cents(amountCents) / 100);
}
...
function applicationDecision(status) {
  if (APPLICATION_APPROVAL_STATUSES.has(status)) return "approved";
  return status === "rejected" ? "rejected" : null;
}

function normalizePartnerApplication(itemInput) {
  const item = itemInput && typeof itemInput === "object" ? itemInput : {};
  const derivedDecision = applicationDecision(item.status);
  const decisionStatus = APPLICATION_DECISIONS.has(item.decisionStatus) ? item.decisionStatus : derivedDecision;
  return {
    ...item,
    intakeMode: item.type === "vendor" && item.intakeMode === "interest" ? "interest" : "application",
    consentPreferenceVersion: Math.max(1, Math.round(Number(item.consentPreferenceVersion) || 1)),
    consentPreferenceUpdatedAt: iso(item.consentPreferenceUpdatedAt, iso(item.consentCapturedAt)),
    consentWithdrawnAt: iso(item.consentWithdrawnAt),
    decisionStatus,
    decisionVersion: Math.max(0, Math.round(Number(item.decisionVersion) || (decisionStatus ? 1 : 0))),
    decisionAt: iso(item.decisionAt)
  };
}

function normalizePartnerFollowup(itemInput) {
  const item = itemInput && typeof itemInput === "object" ? itemInput : {};
  const deliveryOutcomeUnknown = item.deliveryOutcomeUnknown === true || item.status === "delivery_unknown";
  return {
    ...item,
    status: deliveryOutcomeUnknown
      ? "delivery_unknown"
      : FOLLOWUP_STATUSES.has(item.status) ? item.status : "pending",
    deliveryOutcomeUnknown,
    deliveryResolution: FOLLOWUP_DELIVERY_RESOLUTIONS.has(item.deliveryResolution) ? item.deliveryResolution : null,
    deliveryReconciledAt: iso(item.deliveryReconciledAt),
    deliveryReconciledBy: text(item.deliveryReconciledBy, 120) || null,
    deliveryResolutionNote: text(item.deliveryResolutionNote, 1000) || null,
    deliveryCancellationRequestedAt: iso(item.deliveryCancellationRequestedAt),
    deliveryCancellationRequestedBy: text(item.deliveryCancellationRequestedBy, 120) || null,
    deliveryCancellationReason: text(item.deliveryCancellationReason, 1000) || null
  };
}
...
export function normalizePartnerOperations(input) {
  ...
  return {
    ...
    ...base,
    applications: Array.isArray(base.applications) ? base.applications.map(normalizePartnerApplication) : [],
    ...
    followups: Array.isArray(base.followups) ? base.followups.map(normalizePartnerFollowup) : [],
    tasks: Array.isArray(base.tasks) ? base.tasks.map(item => ({
      ...item,
      assignmentVersion: taskAssignmentVersion(item),
      assignmentNoticeVersion: taskAssignmentNoticeVersion(item),
      scheduleVersion: taskScheduleVersion(item),
      acknowledgedAt: iso(item?.acknowledgedAt),
      assigneeUpdates: Array.isArray(item?.assigneeUpdates) ? item.assigneeUpdates.slice(-50) : []
    })) : [],
...
function vendorApplicationOpening(offeringInput, categoryInput) {
  const offering = offeringInput && typeof offeringInput === "object" ? offeringInput : {};
  const id = text(offering.id, 100).toLowerCase();
  const category = text(categoryInput, 40).toLowerCase();
  const categories = list(offering.categories, 20).map(item => item.toLowerCase()).sort();
  const amount = Number(offering.amount);
  if (!id || offering.active === false || offering.intakeMode !== "application"
    || !Number.isInteger(amount) || amount < 0 || !categories.includes(category)) return null;
  const name = text(offering.name, 120);
  const publicLabel = text(offering.publicLabel, 120);
  if (!name || !publicLabel) return null;
  return {
    id,
    name,
    publicLabel,
    amount,
    categories,
    sourceVersion: `opening:${id}:${amount}:${JSON.stringify([name, publicLabel, categories])}`
  };
}

function currentVendorApplicationOpening(application, offerings) {
  if (application?.type !== "vendor" || application?.intakeMode !== "interest") return null;
  const offeringId = text(application.offeringId, 100).toLowerCase();
  const offering = (Array.isArray(offerings) ? offerings : []).find(item => text(item?.id, 100).toLowerCase() === offeringId);
  return vendorApplicationOpening(offering, application.category);
}
...
function defaultDueDates(type, at, intakeMode = "application") {
  const base = new Date(at);
  const addDays = days => new Date(base.getTime() + days * 86_400_000).toISOString();
  if (type === "vendor" && intakeMode === "interest") {
    return [["Review vendor interest", addDays(5), "interest_review", "operations"]];
  }
  return type === "vendor"
    ? [
...
function applicationReference(type, id) {
  return `TSF-${type === "vendor" ? "V" : "S"}-${String(id).replace(/\D/g, "").slice(-6).padStart(6, "0")}`;
}

function allocateApplicationIdentity(doc, type, idFactory) {
  const idPrefix = type === "vendor" ? "vapp" : "sapp";
  const ids = new Set(doc.applications.map(item => String(item.id || "")));
  const references = new Set(doc.applications.map(item => String(item.reference || "").toUpperCase()));
  for (let attempt = 0; attempt < APPLICATION_IDENTITY_MAX_ATTEMPTS; attempt += 1) {
    const id = text(idFactory(idPrefix), 120);
    if (!id || ids.has(id)) continue;
    const reference = applicationReference(type, id);
  ...
  return {
    ok: false,
    retryable: true,
    error: "A unique application reference could not be allocated. Retry the submission."
  };
}
...
export function createPartnerApplication(docInput, input, options = {}) {
  ...
  if (!Number.isFinite(new Date(now).getTime())) {
    return { ok: false, error: "Application intake time is invalid." };
  }
  ...
  const requestedIntakeMode = text(input?.intakeMode || "application", 20).toLowerCase();
  if (type === "vendor" && !PARTNER_INTAKE_MODES.has(requestedIntakeMode)) {
    return { ok: false, error: "Vendor intake mode must be interest or application." };
  }
  const intakeMode = type === "vendor" ? requestedIntakeMode : "application";
  if (intakeMode === "interest" && (cents(input?.requestedAmountCents) > 0 || cents(input?.expectedAmountCents) > 0)) {
    return { ok: false, error: "Vendor interest cannot create a financial obligation." };
  }
  ...
  const identity = allocateApplicationIdentity(doc, type, idFactory);
  if (!identity.ok) return identity;
  const { id, reference } = identity;
  ...
  const application = {
    ...
    consentNoticeVersion: text(input?.consentNoticeVersion, 120) || null,
    consentCapturedAt: iso(input?.consentCapturedAt, input?.consentToContact === true ? now : null),
    consentWithdrawnAt: null,
    consentPreferenceVersion: 1,
    consentPreferenceUpdatedAt: input?.consentToContact === true ? now : null,
  ...
  const dueDates = defaultDueDates(type, now, intakeMode).map(([label, dueAt, kind, assigneeTeam]) => milestone(
    idFactory,
    id,
    label,
    dueAt,
  ...
  const task = {
    id: idFactory("task"), title: `Review ${organizationName} ${type === "vendor" && intakeMode === "interest" ? "vendor interest" : `${type} application`}`,
  ...
  const isVendorApplication = type === "vendor" && intakeMode === "application";
  ...
  const vendorProfile = isVendorApplication ? {
  ...
  const vendorRequirements = isVendorApplication ? vendorRequirementTemplates(application.category).map(([code, label]) => ({
  ...
  const vendorAssignment = isVendorApplication ? {
  ...
  const next = {
    ...
    activity: [...doc.activity, activity(idFactory, "application.created", "application", id, now, options.actorId ?? "public", { type, intakeMode })].slice(-1000)
  };
...
export function updatePartnerContactPreference(docInput, applicationId, input, options = {}) {
  const doc = normalizePartnerOperations(docInput);
  const index = doc.applications.findIndex(item => item.id === applicationId);
  if (index < 0) return { ok: false, error: "Application not found." };
  if (typeof input?.consentToContact !== "boolean") return { ok: false, error: "Choose whether SandFest may contact you about this application." };
  const applicationBefore = doc.applications[index];
  const consentToContact = input.consentToContact;
  const noticeVersion = text(options.noticeVersion, 120);
  if (consentToContact && !noticeVersion) return { ok: false, error: "The current contact notice is unavailable." };
  const currentVersion = Math.max(1, Math.round(Number(applicationBefore.consentPreferenceVersion) || 1));
  const expectedVersion = Number(input.expectedVersion);
  if (!Number.isInteger(expectedVersion)) return { ok: false, error: "The current message preference version is required." };
  const samePreference = applicationBefore.consentToContact === consentToContact
    && (!consentToContact || applicationBefore.consentNoticeVersion === noticeVersion);
  if (samePreference) {
    return { ok: true, changed: false, replay: true, application: applicationBefore, dismissedFollowups: 0, doc };
  }
  if (expectedVersion !== currentVersion) {
    return { ok: false, conflict: true, error: "Message preferences changed in another session. Reload the private portal and try again." };
  }
  ...
  const application = {
    ...applicationBefore,
    consentToContact,
    consentNoticeVersion: consentToContact ? noticeVersion : applicationBefore.consentNoticeVersion,
    consentCapturedAt: consentToContact ? now : applicationBefore.consentCapturedAt,
    consentWithdrawnAt: consentToContact ? null : now,
    consentPreferenceVersion: currentVersion + 1,
    consentPreferenceUpdatedAt: now,
    updatedAt: now
  };
  const applications = doc.applications.slice();
  applications[index] = application;
  const dismissibleStatuses = new Set(["pending", "draft_ready", "approved", "queued", "sending", "failed", "delivery_unknown"]);
  let dismissedFollowups = 0;
  const followups = consentToContact ? doc.followups : doc.followups.map(item => {
    if (item.applicationId !== applicationId || !dismissibleStatuses.has(item.status)) return item;
    const held = requestUnknownFollowupCancellation(item, "Contact permission was withdrawn before delivery verification.", "partner", now);
    if (held) return held;
    ...
    return {
      ...
      automationDecision: "contact_permission_withdrawn",
      ...
      dismissedAt: now,
      dismissedBy: "partner",
      lastError: "Contact permission was withdrawn before delivery.",
      updatedAt: now
    };
  });
  const actorId = options.actorId ?? "partner";
  const next = {
    ...doc,
    lastUpdated: now,
    applications,
    followups,
    activity: [...doc.activity, activity(idFactory, "application.contact_preference.updated", "application", applicationId, now, actorId, {
      consentToContact,
      consentPreferenceVersion: application.consentPreferenceVersion,
      dismissedFollowups
    })].slice(-1000)
  };
  return { ok: true, changed: true, replay: false, application, dismissedFollowups, doc: next };
}
...
export function rotatePartnerPortalAccess(docInput, applicationId, options = {}) {
  ...
  const index = doc.applications.findIndex(item => item.id === applicationId);
  if (index < 0) return { ok: false, error: "Application not found." };
  if (doc.followups.some(item => item.applicationId === applicationId && ["sending", "delivery_unknown"].includes(item.status))) {
    return { ok: false, error: "A portal message is being sent or awaiting provider verification. Resolve it before rotating access." };
  }
  const now = options.now ?? new Date().toISOString();
  const idFactory = options.idFactory ?? (prefix => `${prefix}_${crypto.randomUUID()}`);
  const portalAccessIdFactory = options.portalAccessIdFactory ?? (() => crypto.randomUUID());
...
export function requestPartnerPortalRecovery(docInput, input, options = {}) {
  const doc = normalizePartnerOperations(docInput);
  const reference = text(input?.reference, 80).toUpperCase();
  const contactEmail = email(input?.contactEmail);
  const application = contactEmail
    ? doc.applications.find(item => String(item.reference || "").toUpperCase() === reference
      && String(item.contactEmail || "").toLowerCase() === contactEmail
      && item.consentToContact === true)
    : null;
  if (!application) {
    return { ok: true, changed: false, matched: false, duplicate: false, followup: null, doc };
  }

  const now = options.now ?? new Date().toISOString();
  const nowMs = new Date(now).getTime();
  if (!Number.isFinite(nowMs)) return { ok: false, error: "Recovery request time is invalid." };
  const portalUrl = typeof options.portalUrlForApplication === "function"
    ? text(options.portalUrlForApplication(application), 1000)
    : "";
  if (!portalUrl) return { ok: false, error: "Partner portal access is temporarily unavailable." };

  const sourcePrefix = `portal:${Number(application.portalAccessVersion || 1)}:`;
  const existing = [...doc.followups].reverse().find(item => item.applicationId === application.id
    && item.kind === "portal_access_recovery"
    && String(item.sourceVersion || "").startsWith(sourcePrefix)
    && item.status !== "dismissed"
    && new Date(item.createdAt || item.generatedAt || 0).getTime() > nowMs - PARTNER_PORTAL_RECOVERY_WINDOW_MS);
  if (existing) {
    return { ok: true, changed: false, matched: true, duplicate: true, followup: existing, doc };
  }

  const idFactory = options.idFactory ?? (prefix => `${prefix}_${crypto.randomUUID()}`);
  const sourceVersion = `${sourcePrefix}requested:${now}`;
  const followup = {
    id: idFactory("followup"),
    applicationId: application.id,
    kind: "portal_access_recovery",
    workflowKey: `portal_access_recovery:${application.id}`,
    sourceVersion,
    channel: "email",
    recipient: application.contactEmail,
    status: "approved",
    dueAt: now,
    subject: "Your Texas SandFest partner portal link",
    body: `Hello ${application.contactName},\n\nHere is the current private link for ${application.organizationName}'s Texas SandFest partner application (${application.reference}):\n${portalUrl}\n\nIf you did not request this message, you can ignore it. The link only provides access to this application.\n\nTexas SandFest`,
    portalUrl,
    approvedBy: "public:portal_access_recovery",
    approvedAt: now,
    sentAt: null,
    queuedAt: null,
    provider: null,
    ...
    deliveryReconciledAt: null,
    deliveryReconciledBy: null,
    deliveryResolutionNote: null,
    deliveryCancellationRequestedAt: null,
    deliveryCancellationRequestedBy: null,
    deliveryCancellationReason: null,
    deliveryAttempts: 0,
    lastAttemptAt: null,
    lastError: null,
    generatedAt: now,
  ...
  return {
    ...
    doc: {
      ...doc,
      lastUpdated: now,
      followups: [...doc.followups, followup],
      activity: [...doc.activity, activity(
        idFactory,
        "application.portal_access_requested",
        "application",
        application.id,
        now,
        "public",
...
export function updatePartnerApplication(docInput, applicationId, patch, options = {}) {
  ...
  if (current.type === "vendor" && current.intakeMode === "interest"
    && patch.expectedAmountCents !== undefined && cents(patch.expectedAmountCents) > 0) {
    return { ok: false, error: "Vendor interest cannot create a financial obligation." };
  }
  ...
  const currentDecision = applicationDecision(current.status);
  const nextDecision = applicationDecision(status);
  const decisionChanged = status !== current.status && currentDecision !== nextDecision;
  const decisionVersion = Math.max(0, Math.round(Number(current.decisionVersion) || (currentDecision ? 1 : 0))) + (decisionChanged ? 1 : 0);
  const application = {
    ...current,
    status,
    ...
    decisionStatus: decisionChanged ? nextDecision : current.decisionStatus ?? currentDecision,
    decisionVersion,
    decisionAt: decisionChanged ? (nextDecision ? now : null) : current.decisionAt ?? null,
    updatedAt: now
  };
  const applications = doc.applications.slice();
  applications[index] = application;
  const baseDoc = {
    ...
    activity: [...doc.activity, activity(idFactory, "application.updated", "application", applicationId, now, options.actorId ?? "admin", {
      status,
      decisionStatus: application.decisionStatus,
      decisionVersion: application.decisionVersion
    })].slice(-1000)
  };
  if (!decisionChanged || application.intakeMode === "interest") {
    return { ok: true, application, followup: null, dismissedFollowups: 0, doc: baseDoc };
  }
  const workflowKey = partnerWorkflowKey("application_decision", applicationId);
  if (!nextDecision) {
    const dismissed = dismissPartnerWorkflowFollowups(baseDoc, workflowKey, "The application decision was reopened before delivery.", { now });
    return { ok: true, application, followup: null, dismissedFollowups: dismissed.dismissed, doc: dismissed.doc };
  }
  const applicationLabel = application.type === "sponsor" ? "sponsorship application" : "vendor application";
  const approved = nextDecision === "approved";
  const portalUrl = typeof options.portalUrlForApplication === "function" ? options.portalUrlForApplication(application) : options.portalUrl;
  const notice = upsertPartnerWorkflowFollowup(baseDoc, application, {
    kind: approved ? "application_approved" : "application_rejected",
    workflowKey,
    sourceVersion: `decision:${application.decisionVersion}:${nextDecision}`,
    manualReviewRequired: !approved,
    subject: approved
      ? `Texas SandFest ${applicationLabel} approved - ${application.reference}`
      : `Texas SandFest ${applicationLabel} update - ${application.reference}`,
    body: approved
      ? `Hello ${application.contactName},\n\nYour ${applicationLabel} for ${application.organizationName} has been approved for Texas SandFest.\n\nNext steps, payment dates, onboarding requirements, and delivery status are available in the private partner portal.\n\nTexas SandFest`
      : `Hello ${application.contactName},\n\nThank you for submitting the ${applicationLabel} for ${application.organizationName}. We are unable to approve it for the current Texas SandFest event.\n\nThe SandFest team will contact you if additional context or next steps are available.\n\nTexas SandFest`
  }, { ...options, now, idFactory, portalUrl });
  return {
    ok: true,
    application,
    followup: notice.followup ?? null,
    followupChanged: notice.changed === true,
    dismissedFollowups: notice.dismissed ?? 0,
    doc: notice.doc
  };
}
...
export function updatePartnerBrandProfile(docInput, applicationId, input = {}, options = {}) {
  ...
  const dismissed = dismissPartnerWorkflowFollowups(baseDoc, partnerWorkflowKey("sponsor_brand_profile", applicationId), "Sponsor submitted a revised brand profile; the prior change request is stale.", { now });
  return {
    ok: true,
    profile,
...
export function reviewPartnerBrandProfile(docInput, applicationId, input = {}, options = {}) {
  ...
  const workflowKey = partnerWorkflowKey("sponsor_brand_profile", applicationId);
  const notification = status === "changes_requested"
    ? upsertPartnerWorkflowFollowup(baseDoc, sponsor.application, {
      kind: "sponsor_brand_changes",
      workflowKey,
      brandProfileId: profile.id,
      sourceVersion: `review:${profile.updatedAt}`,
      subject: `Texas SandFest sponsor brand profile changes - ${sponsor.application.reference}`,
      body: `Hello ${sponsor.application.contactName},\n\nThe SandFest team reviewed ${profile.displayName}'s brand profile and needs the following update:\n\n${reviewNotes}\n\nPlease revise the profile in the private sponsor portal.\n\nTexas SandFest`
    }, { ...options, now, idFactory })
    : dismissPartnerWorkflowFollowups(baseDoc, workflowKey, "Sponsor brand profile was approved before this notice was delivered.", { now });
  return {
    ok: true,
    profile,
...
export function createPartnerBrandAsset(docInput, applicationId, input = {}, options = {}) {
  ...
  const dismissed = dismissPartnerWorkflowFollowups(baseDoc, partnerWorkflowKey("sponsor_brand_asset", `${applicationId}:${kind}`), "Sponsor submitted a replacement brand asset; the prior change request is stale.", { now });
  return {
    ok: true,
    duplicate: false,
    asset,
...
export function reviewPartnerBrandAsset(docInput, assetId, input = {}, options = {}) {
  ...
  const workflowKey = partnerWorkflowKey("sponsor_brand_asset", `${asset.applicationId}:${asset.kind}`);
  const notification = ["changes_requested", "rejected"].includes(status)
    ? upsertPartnerWorkflowFollowup(baseDoc, application, {
      kind: "sponsor_brand_changes",
      workflowKey,
      brandAssetId: asset.id,
      sourceVersion: `review:${asset.updatedAt}`,
      subject: `Texas SandFest sponsor brand asset changes - ${application.reference}`,
      body: `Hello ${application.contactName},\n\nThe SandFest team reviewed ${asset.label} for ${application.organizationName} and ${status === "rejected" ? "could not accept it" : "needs an updated version"}.\n\n${reviewNotes}\n\nPlease submit a replacement in the private sponsor portal.\n\nTexas SandFest`
    }, { ...options, now, idFactory })
    : dismissPartnerWorkflowFollowups(baseDoc, workflowKey, `${asset.label} no longer needs sponsor action.`, { now });
  return {
    ok: true,
    asset,
    followup: notification.followup ?? null,
...
export function createPartnerDeliverable(docInput, applicationId, input = {}, options = {}) {
  ...
  const deliverableId = idFactory("deliverable");
  const description = text(input.description, 1000);
  const ownerId = text(input.ownerId, 100) || null;
  const existing = doc.deliverables.find(item => item.id === deliverableId);
  if (existing) {
    const replay = existing.applicationId === applicationId
      && existing.packageId === sponsor.application.packageId
      && existing.source === "custom"
      && existing.label === label
      && existing.description === description
      && existing.status === "planned"
      && existing.ownerId === ownerId
      && (existing.dueAt ?? null) === (due.value ?? null)
      && existing.createdBy === actorId;
    return replay
      ? { ok: true, replay: true, deliverable: existing, doc }
      : { ok: false, code: "IDEMPOTENCY_CONFLICT", error: "This deliverable request key was already used for different fulfillment details." };
  }
  const deliverable = {
    id: deliverableId,
    applicationId,
  ...
  return {
    ...
    doc: {
      ...
      activity: [...doc.activity, activity(idFactory, "deliverable.created", "deliverable", deliverable.id, now, actorId, { applicationId })].slice(-1000)
    }
  };
}
...
export function updatePartnerDeliverable(docInput, deliverableId, input = {}, options = {}) {
  ...
  const workflowKey = partnerWorkflowKey("sponsor_deliverable", deliverableId);
  let notification = { doc: baseDoc, dismissed: 0 };
  if (proofChanged && hasProof) {
    notification = upsertPartnerWorkflowFollowup(baseDoc, application, {
      kind: "sponsor_deliverable_review",
      workflowKey,
      deliverableId: deliverable.id,
      sourceVersion: `proof:${deliverable.proofVersion}`,
      subject: `Texas SandFest sponsor proof ready - ${application.reference}`,
      body: `Hello ${application.contactName},\n\nProof for ${deliverable.label} is ready for your review.\n\nPlease approve it or request changes in the private sponsor portal.\n\nTexas SandFest`
    }, { ...options, now, idFactory });
  } else if ((proofChanged && !hasProof) || status === "cancelled") {
    notification = dismissPartnerWorkflowFollowups(baseDoc, workflowKey, "This sponsor deliverable no longer needs partner review.", { now });
  }
  return {
    ok: true,
    deliverable,
...
export function reviewPartnerDeliverable(docInput, deliverableId, input = {}, options = {}) {
  ...
  const dismissed = dismissPartnerWorkflowFollowups(baseDoc, partnerWorkflowKey("sponsor_deliverable", deliverableId), "Sponsor completed the current deliverable review.", { now });
  return {
    ok: true,
    deliverable,
    dismissedFollowups: dismissed.dismissed,
...
function vendorApplication(doc, applicationId) {
  ...
  if (application.intakeMode === "interest") return { ok: false, error: "Vendor onboarding opens only after an application is created." };
  return { ok: true, application };
}

const ACTIVE_WORKFLOW_FOLLOWUP_STATUSES = new Set(["pending", "draft_ready", "approved", "queued", "sending", "failed", "delivery_unknown"]);

function requestUnknownFollowupCancellation(item, reason, actorId, now) {
  if (item.status !== "delivery_unknown" || item.deliveryOutcomeUnknown !== true) return null;
  return {
    ...item,
    deliveryCancellationRequestedAt: item.deliveryCancellationRequestedAt || now,
    deliveryCancellationRequestedBy: item.deliveryCancellationRequestedBy || actorId,
    deliveryCancellationReason: text(reason, 1000) || "The message is no longer needed after provider verification.",
    updatedAt: now
  };
}

function partnerWorkflowKey(kind, entityId) {
  return `${kind}:${entityId}`;
}

function dismissPartnerWorkflowFollowups(doc, workflowKey, reason, options = {}) {
  const now = options.now ?? new Date().toISOString();
  let dismissed = 0;
  const followups = doc.followups.map(item => {
    if (item.workflowKey !== workflowKey || !ACTIVE_WORKFLOW_FOLLOWUP_STATUSES.has(item.status)) return item;
    const held = requestUnknownFollowupCancellation(item, reason, options.actorId ?? "automation", now);
    if (held) return held;
    if (item.status === "sending" && item.providerSubmissionStartedAt) return item;
    dismissed += 1;
    return {
      ...
      providerSubmissionStartedAt: null,
      automationJobId: null,
      lastError: text(reason, 500) || "The partner completed this workflow step before delivery.",
      updatedAt: now
    };
  });
...
function upsertPartnerWorkflowFollowup(doc, application, input, options = {}) {
  const now = options.now ?? new Date().toISOString();
  const idFactory = options.idFactory ?? (prefix => `${prefix}_${crypto.randomUUID()}`);
  ...
  const activeSubmission = [...doc.followups].reverse().find(item => item.workflowKey === workflowKey
    && (item.status === "delivery_unknown" || (item.status === "sending" && item.providerSubmissionStartedAt)));
  ...
  const followup = {
    ...
    workflowKey,
    sourceVersion,
    brandProfileId: text(input.brandProfileId, 120) || null,
    brandAssetId: text(input.brandAssetId, 120) || null,
    deliverableId: text(input.deliverableId, 120) || null,
    channel: "email",
    recipient: application.contactEmail,
    ...
    manualReviewRequiredAt: input.manualReviewRequired === true ? now : null,
    approvedBy: null,
    approvedAt: null,
    sentAt: null,
    queuedAt: null,
...
export function updateVendorProfile(docInput, applicationId, input = {}, options = {}) {
  ...
  const dismissed = dismissPartnerWorkflowFollowups(baseDoc, partnerWorkflowKey("vendor_profile", applicationId), "Vendor submitted a revised operating profile; the prior change request is stale.", { now });
  return {
    ok: true,
    profile,
    dismissedFollowups: dismissed.dismissed,
...
export function reviewVendorProfile(docInput, applicationId, input = {}, options = {}) {
  ...
  const workflowKey = partnerWorkflowKey("vendor_profile", applicationId);
  const notification = status === "changes_requested"
    ? upsertPartnerWorkflowFollowup(baseDoc, application, {
    ...
    : dismissPartnerWorkflowFollowups(baseDoc, workflowKey, "Vendor profile was approved before this notice was delivered.", { now });
  return {
    ok: true,
    profile,
    followup: notification.followup ?? null,
...
export function createVendorDocument(docInput, applicationId, requirementId, input = {}, options = {}) {
  ...
  const dismissed = dismissPartnerWorkflowFollowups(baseDoc, partnerWorkflowKey("vendor_requirement", requirementId), "Vendor submitted replacement evidence; the prior change request is stale.", { now });
  return {
    ok: true,
    duplicate: false,
    document,
...
export function reviewVendorRequirement(docInput, requirementId, input = {}, options = {}) {
  ...
  const workflowKey = partnerWorkflowKey("vendor_requirement", requirementId);
  const notification = ["changes_requested", "expired"].includes(status)
    ...
    : ["approved", "waived"].includes(status)
      ? dismissPartnerWorkflowFollowups(baseDoc, workflowKey, `${current.label} was cleared before this notice was delivered.`, { now })
...
export function updateVendorAssignment(docInput, applicationId, input = {}, options = {}) {
  ...
  const workflowKey = partnerWorkflowKey("vendor_assignment", applicationId);
  let notification = { doc: baseDoc, dismissed: 0 };
  if (assignment.status === "scheduled" && (scheduleChanged || enteringScheduled)) {
    notification = upsertPartnerWorkflowFollowup(baseDoc, vendor.application, {
...
export function confirmVendorAssignment(docInput, applicationId, options = {}) {
  ...
  const dismissed = dismissPartnerWorkflowFollowups(baseDoc, partnerWorkflowKey("vendor_assignment", applicationId), "Vendor confirmed the current booth and load-in assignment.", { now });
  return {
    ok: true,
    assignment,
    dismissedFollowups: dismissed.dismissed,
...
export function beginPartnerPaymentCheckout(docInput, applicationId, invoiceId, options = {}) {
  ...
  const provider = text(options.provider, 40).toLowerCase() || "stripe";
  const currentIndex = doc.paymentCheckouts.findIndex(item => item.invoiceId === invoice.id
    && item.provider === provider
    && item.amountCents === amountCents
    && ["creating", "open", "failed"].includes(item.status)
    && checkoutExpiresAt(item, nowMs));
  const current = currentIndex >= 0 ? doc.paymentCheckouts[currentIndex] : null;
  ...
  if (current) {
    ...
    paymentCheckouts[currentIndex] = checkout;
    return {
      ok: true,
      changed: true,
      duplicate: true,
      ...
      doc: {
        ...
        lastUpdated: now,
        paymentCheckouts,
        activity: [...doc.activity, activity(idFactory, "payment_checkout.retried", "payment_checkout", checkout.id, now, options.actorId ?? "partner", {
  ...
  const paymentCheckouts = doc.paymentCheckouts.map(item => ["creating", "open", "failed"].includes(item.status) && !checkoutExpiresAt(item, nowMs)
...
export function activatePartnerPaymentCheckout(docInput, checkoutId, providerSession, options = {}) {
  ...
  const currentCheckoutUrl = stripeHostedCheckoutUrl(current.checkoutUrl);
  if (current.status === "open" && current.providerSessionId === providerSession?.id && currentCheckoutUrl) {
    return { ok: true, duplicate: true, checkout: { ...current, checkoutUrl: currentCheckoutUrl }, doc };
  }
  if (!["creating", "failed"].includes(current.status)) return { ok: false, error: "Payment checkout is not awaiting a Stripe session." };
  const providerSessionId = text(providerSession?.id, 160);
  const checkoutUrl = stripeHostedCheckoutUrl(providerSession?.url);
  if (!/^cs_[A-Za-z0-9_]+$/.test(providerSessionId) || !checkoutUrl) return { ok: false, error: "Stripe returned an invalid checkout session." };
  const now = options.now ?? new Date().toISOString();
  ...
  return { ok: true, duplicate: false, recovered: current.status === "failed", checkout, doc: { ...doc, lastUpdated: now, paymentCheckouts } };
}
...
export function failPartnerPaymentCheckout(docInput, checkoutId, error, options = {}) {
  ...
  if (current.status !== "creating") {
    return { ok: true, changed: false, duplicate: true, checkout: current, doc };
  }
  const now = options.now ?? new Date().toISOString();
  const checkout = {
    ...current,
  ...
  const paymentCheckouts = doc.paymentCheckouts.slice();
  paymentCheckouts[index] = checkout;
  return { ok: true, changed: true, duplicate: false, checkout, doc: { ...doc, lastUpdated: now, paymentCheckouts } };
}
...
export function recordPartnerPayment(docInput, applicationId, input, options = {}) {
  ...
  const application = doc.applications.find(item => item.id === applicationId);
  if (!application) return { ok: false, error: "Application not found." };
  if (application.type === "vendor" && application.intakeMode === "interest") {
    return { ok: false, error: "Vendor interest cannot accept payments." };
  }
  ...
  const idFactory = options.idFactory ?? (prefix => `${prefix}_${crypto.randomUUID()}`);
  const externalRef = text(input?.externalRef, 160);
  if (!externalRef) return { ok: false, error: "Payment reference is required for accounting and retry protection." };
  ...
  if (duplicate) {
    const requestedInvoiceId = text(input?.invoiceId, 100) || null;
    if (cents(duplicate.amountCents) !== amountCents
      || String(duplicate.status || "succeeded").toLowerCase() !== status
      || (requestedInvoiceId && duplicate.invoiceId !== requestedInvoiceId)) {
      return { ok: false, conflict: true, error: "Payment reference was already used for different payment details." };
    }
...
function paymentReceiptSourceVersion(payment) {
  return `payment:${payment.id}:received`;
}

function paymentAdjustmentSourceVersion(payment) {
  const status = text(payment?.status, 40).toLowerCase();
  const changedAt = iso(payment?.reversedAt || payment?.updatedAt || payment?.createdAt) || "unknown";
  return `payment:${payment.id}:adjustment:${status}:${cents(payment?.refundedAmountCents)}:${changedAt}`;
}

function currentPaymentNotice(payment) {
  if (!payment?.id || Number(payment.noticePolicyVersion) !== 1) return null;
  const refundedAmountCents = cents(payment.refundedAmountCents);
  if (["partially_refunded", "refunded", "voided"].includes(payment.status) || refundedAmountCents > 0) {
    return { kind: "payment_adjustment", sourceVersion: paymentAdjustmentSourceVersion(payment) };
  }
  if (payment.status === "succeeded") {
    return { kind: "payment_received", sourceVersion: paymentReceiptSourceVersion(payment) };
  }
  return null;
}
...
export function reconcilePartnerStripePayment(docInput, input = {}, options = {}) {
  ...
  const paymentMethod = text(options.paymentMethod, 40).toLowerCase() || "stripe";
  const paymentReferencePrefix = text(options.paymentReferencePrefix, 40).toLowerCase() || "stripe";
  const paymentResult = recordPartnerPayment(doc, checkout.applicationId, {
    invoiceId: checkout.invoiceId,
    amountCents,
    method: paymentMethod,
    status: "succeeded",
    externalRef: `${paymentReferencePrefix}:${paymentIntentId}`,
    providerEventId,
    providerCheckoutId: providerSessionId,
    paymentIntentId,
    receivedAt: input.receivedAt,
    notes: text(options.paymentNotes, 500) || `Stripe partner checkout ${checkout.id}`
  }, {
...
export function createPartnerInvoice(docInput, applicationId, input = {}, options = {}) {
  ...
  if (!application) return { ok: false, error: "Application not found." };
  if (application.type === "vendor" && application.intakeMode === "interest") {
    return { ok: false, error: "Vendor interest cannot be invoiced." };
  }
  ...
  const now = options.now ?? new Date().toISOString();
  const nowMs = new Date(now).getTime();
  if (!Number.isFinite(nowMs)) return { ok: false, error: "Invoice creation time is invalid." };
  const idFactory = options.idFactory ?? (prefix => `${prefix}_${crypto.randomUUID()}`);
  const defaultDueAt = new Date(nowMs + 30 * 86_400_000).toISOString();
...
export function createPartnerTask(docInput, input, options = {}) {
  ...
  const description = text(input?.description, 1000);
  const priority = TASK_PRIORITIES.has(input?.priority) ? input.priority : "normal";
  const relatedEntityType = text(input?.relatedEntityType, 60) || null;
  const relatedEntityId = text(input?.relatedEntityId, 100) || null;
  const existing = doc.tasks.find(item => item.id === taskId);
  if (existing) {
    const replay = existing.title === title
      && existing.description === description
      && existing.priority === priority
      && existing.assigneeType === assignment.assigneeType
      && existing.assigneeId === assignment.assigneeId
      && existing.assigneeName === assignment.assigneeName
      && existing.assigneeRole === assignment.assigneeRole
      && existing.relatedEntityType === relatedEntityType
      && existing.relatedEntityId === relatedEntityId
      && (existing.dueAt ?? null) === (dueAt.value ?? null)
      && existing.createdBy === actorId;
    return replay
      ? { ok: true, replay: true, task: existing, doc }
      : { ok: false, code: "IDEMPOTENCY_CONFLICT", error: "This task request key was already used for different delegation details." };
  }
  const task = {
    ...
    scheduleVersion: 1,
    lastAssignmentNoticeRequestId: null,
    lastAssignmentNoticeRequestedAt: null,
    lastAssignmentNoticeRequestedBy: null,
    createdBy: actorId,
    createdAt: now,
    updatedAt: now,
    startedAt: null,
...
export function updatePartnerTask(docInput, taskId, patch, options = {}) {
  ...
  const task = {
    ...
    assignmentNoticeVersion: assignmentChanged ? 0 : taskAssignmentNoticeVersion(current),
    scheduleVersion: scheduleChanged ? taskScheduleVersion(current) + 1 : taskScheduleVersion(current),
    lastAssignmentNoticeRequestId: assignmentChanged ? null : current.lastAssignmentNoticeRequestId ?? null,
    lastAssignmentNoticeRequestedAt: assignmentChanged ? null : current.lastAssignmentNoticeRequestedAt ?? null,
    lastAssignmentNoticeRequestedBy: assignmentChanged ? null : current.lastAssignmentNoticeRequestedBy ?? null,
    acknowledgedAt: assignmentChanged ? null : current.acknowledgedAt ?? null,
    updatedAt: now,
...
export function updatePartnerTaskFromAssignee(docInput, taskId, input, options = {}) {
  const doc = normalizePartnerOperations(docInput);
  const current = doc.tasks.find(item => item.id === taskId);
  if (!current) return { ok: false, error: "Task assignment not found or access link invalid." };
  if (!current.assigneeId || current.assigneeType === "unassigned") return { ok: false, error: "Task assignment not found or access link invalid." };
  const action = text(input?.action, 30).toLowerCase();
  if (!TASK_ASSIGNEE_ACTIONS.has(action)) return { ok: false, error: "Choose acknowledge, start, report blocker, or complete." };
  const note = text(input?.note, 500);
  if (action === "block" && !note) return { ok: false, error: "Describe the blocker so the operations team can respond." };
  const requestId = text(options.requestId, 200);
  if (requestId && (requestId.length < 16 || !/^[A-Za-z0-9][A-Za-z0-9._:-]+$/.test(requestId))) {
    return { ok: false, error: "A valid task update request ID is required." };
  }
  const priorRequest = requestId
    ? (current.assigneeUpdates || []).find(item => item.requestId === requestId)
    : null;
  if (priorRequest) {
    if (
      Number(priorRequest.assignmentVersion || 1) !== taskAssignmentVersion(current)
      || priorRequest.action !== action
      || String(priorRequest.note || "") !== note
    ) {
      return { ok: false, conflict: true, error: "This task update request ID was already used for different details." };
    }
    return { ok: true, replay: true, changed: false, task: current, assigneeUpdate: priorRequest, doc };
  }
  if (current.status === "cancelled") return { ok: false, conflict: true, error: "This task was cancelled and can no longer be updated." };
  if (current.status === "done") {
    if (action === "complete") return { ok: true, replay: true, changed: false, task: current, doc };
    return { ok: false, conflict: true, error: "This task is already complete." };
  }
  if (action === "acknowledge" && current.acknowledgedAt) {
    return { ok: true, replay: true, changed: false, task: current, doc };
  }
  if (action === "start" && current.status === "in_progress" && !note) {
    return { ok: true, replay: true, changed: false, task: current, doc };
  }

  const now = options.now ?? new Date().toISOString();
  const idFactory = options.idFactory ?? (prefix => `${prefix}_${crypto.randomUUID()}`);
  const status = action === "start" ? "in_progress" : action === "block" ? "blocked" : action === "complete" ? "done" : current.status;
  const actorId = `task-assignee:${current.assigneeType}:${current.assigneeId}`;
  const updated = updatePartnerTask(doc, taskId, { status }, { ...options, now, idFactory, actorId });
  if (!updated.ok) return updated;

  const assigneeUpdate = {
    id: idFactory("task_update"),
    assignmentVersion: taskAssignmentVersion(updated.task),
    action,
    note: note || null,
    at: now,
    ...(requestId ? { requestId } : {})
  ...
  const task = {
    ...
    lastAssigneeUpdateAt: now,
    assigneeUpdates: [...(current.assigneeUpdates || []), assigneeUpdate].slice(-50)
  };
  const tasks = updated.doc.tasks.map(item => item.id === taskId ? task : item);
  const activity = updated.doc.activity.map((item, index, items) => index === items.length - 1 && item.entityType === "task" && item.entityId === taskId
    ? { ...item, type: "task.assignee_updated", detail: { ...item.detail, assigneeAction: action, noteProvided: Boolean(note) } }
    : item);
  return {
    ...updated,
    changed: true,
    task,
    assigneeUpdate,
    doc: { ...updated.doc, tasks, activity }
  };
}

export function requestTaskAssignmentNotice(docInput, taskId, options = {}) {
  const doc = normalizePartnerOperations(docInput);
  const index = doc.tasks.findIndex(item => item.id === taskId);
  if (index < 0) return { ok: false, error: "Task not found." };
  const current = doc.tasks[index];
  ...
  if (!/^[A-Za-z0-9][A-Za-z0-9._:-]{7,199}$/.test(requestId)) {
    return { ok: false, error: "A valid assignment notice request ID is required." };
  }
  if (current.lastAssignmentNoticeRequestId === requestId) {
    return { ok: true, changed: false, replay: true, task: current, doc };
  }
  if (!["open", "in_progress", "blocked"].includes(current.status)) {
    return { ok: false, conflict: true, error: "Only active tasks can send assignment notices." };
  }
  if (!current.assigneeId || current.assigneeType === "unassigned") {
    return { ok: false, error: "Assign this task before sending an assignment notice." };
  }
  if (!taskNotificationRecipient(current, options)) {
    return { ok: false, error: "The current assignee does not have an active notification email." };
  }

  const assignmentPrefix = `assignment:${taskAssignmentVersion(current)}`;
  const currentNotices = doc.followups.filter(item => item.taskId === taskId
    && item.kind === "task_assignment"
    && (item.sourceVersion === assignmentPrefix || String(item.sourceVersion || "").startsWith(`${assignmentPrefix}:notice:`)));
  const pendingNotice = currentNotices.find(item => ["pending", "draft_ready", "approved", "queued", "sending"].includes(item.status));
  if (pendingNotice) {
    return { ok: false, conflict: true, error: "The current assignment notice is already awaiting delivery." };
  }

  const now = options.now ?? new Date().toISOString();
  const idFactory = options.idFactory ?? (prefix => `${prefix}_${crypto.randomUUID()}`);
  const actorId = text(options.actorId, 100) || "admin";
  const previousNoticeVersion = taskAssignmentNoticeVersion(current);
  const task = {
    ...current,
    assignmentNoticeVersion: previousNoticeVersion + 1,
    lastAssignmentNoticeRequestId: requestId,
    lastAssignmentNoticeRequestedAt: now,
    lastAssignmentNoticeRequestedBy: actorId,
    updatedAt: now
  };
  const tasks = doc.tasks.slice();
  tasks[index] = task;
  const baseDoc = {
    ...
    lastUpdated: now,
    tasks,
    activity: [...doc.activity, activity(idFactory, "task.assignment_notice_requested", "task", taskId, now, actorId, {
      assignmentVersion: taskAssignmentVersion(task),
      previousNoticeVersion,
      assignmentNoticeVersion: task.assignmentNoticeVersion
    })].slice(-1000)
  };
  const dismissed = dismissTaskFollowups(baseDoc, taskId, "Assignment notice reissued by staff.", {
    now,
    actorId,
    kinds: ["task_assignment"]
  });
  return {
    ok: true,
    changed: true,
...
function taskAssignmentNoticeVersion(task) {
  return Math.max(0, Math.round(Number(task?.assignmentNoticeVersion || 0)));
}

function taskNotificationDetails(item) {
  const timestamp = item.updatedAt || item.createdAt;
  const state = String(item.deliveryStatus || item.status || "pending").replaceAll("_", " ");
  return `${state}${timestamp ? ` ${new Date(timestamp).toLocaleString("en-US", { timeZone: "America/Chicago", dateStyle: "medium", timeStyle: "short" })}` : ""}`;
}

export function summarizeTaskNotifications(task, followups = []) {
  const assignmentType = task.assigneeType || (task.assigneeId ? "staff" : task.assigneeRole ? "team" : "unassigned");
  if (assignmentType === "unassigned") {
    return { count: 0, latestStatus: null, assignmentLabel: "Assignment notices · not configured", followupLabel: null };
  }
  const taskFollowups = followups
    .filter(item => item.taskId === task.id)
    .sort((a, b) => String(b.updatedAt || b.createdAt).localeCompare(String(a.updatedAt || a.createdAt)));
  const assignmentSource = `assignment:${taskAssignmentVersion(task)}`;
  const assignmentNotices = taskFollowups.filter(item => item.kind === "task_assignment"
    && (item.sourceVersion === assignmentSource || String(item.sourceVersion || "").startsWith(`${assignmentSource}:notice:`)));
  const latestAssignment = assignmentNotices[0] || null;
  const latestFollowup = taskFollowups.find(item => item.kind !== "task_assignment") || null;
  return {
    count: assignmentNotices.length,
    latestStatus: latestAssignment?.status || null,
    assignmentLabel: latestAssignment
      ? `Assignment notices · ${assignmentNotices.length} issued · latest ${taskNotificationDetails(latestAssignment)}`
      : "Assignment notices · awaiting directory",
    followupLabel: latestFollowup
      ? `Latest follow-up · ${String(latestFollowup.kind || "notification").replaceAll("_", " ")} · ${taskNotificationDetails(latestFollowup)}`
      : null
  };
}

function taskAssignmentSourceVersion(task) {
  const assignment = `assignment:${taskAssignmentVersion(task)}`;
  const notice = taskAssignmentNoticeVersion(task);
  return notice > 0 ? `${assignment}:notice:${notice}` : assignment;
}
...
function dismissTaskFollowups(doc, taskId, reason, options = {}) {
  const now = options.now ?? new Date().toISOString();
  const active = new Set(["pending", "draft_ready", "approved", "queued", "sending", "failed", "delivery_unknown"]);
...
export function createOutreachProspect(docInput, input, options = {}) {
  ...
  const idFactory = options.idFactory ?? (prefix => `${prefix}_${crypto.randomUUID()}`);
  const actorId = text(options.actorId, 100) || "admin";
  const prospectId = idFactory("prospect");
  const contactEmail = email(input?.contactEmail);
  ...
  const prospectBase = {
    id: prospectId, organizationName, website, industry: text(input?.industry, 100),
  ...
  const existing = doc.prospects.find(item => item.id === prospectId);
  if (existing) {
    const replay = existing.organizationName === prospectBase.organizationName
      && existing.website === prospectBase.website
      && existing.industry === prospectBase.industry
      && existing.city === prospectBase.city
      && existing.state === prospectBase.state
      && existing.postalCode === prospectBase.postalCode
      && existing.latitude === prospectBase.latitude
      && existing.longitude === prospectBase.longitude
      && existing.contactName === prospectBase.contactName
      && existing.contactEmail === prospectBase.contactEmail
      && existing.status === prospectBase.status
      && existing.communityFit === prospectBase.communityFit
      && existing.ownerId === prospectBase.ownerId
      && existing.nextAction === prospectBase.nextAction
      && existing.nextActionAt === prospectBase.nextActionAt
      && existing.contactBasis === prospectBase.contactBasis
      && JSON.stringify(existing.tags || []) === JSON.stringify(prospectBase.tags)
      && existing.source === prospectBase.source
      && existing.sourceBatch === prospectBase.sourceBatch
      && existing.sourceRow === prospectBase.sourceRow
      && existing.sourceRef === prospectBase.sourceRef
      && existing.sourceUrl === prospectBase.sourceUrl
      && existing.sourceLicense === prospectBase.sourceLicense
      && existing.sourceFetchedAt === prospectBase.sourceFetchedAt
      && existing.createdBy === actorId;
    return replay
      ? { ok: true, replay: true, prospect: existing, doc }
      : { ok: false, code: "IDEMPOTENCY_CONFLICT", error: "This prospect request key was already used for different outreach details." };
  }
  ...
  return {
    ...
    doc: {
      ...
      activity: [...doc.activity, activity(idFactory, "outreach.prospect.created", "prospect", prospect.id, now, actorId, { fitScore: prospect.fitScore })].slice(-1000)
    }
  };
}
...
export function updateOutreachProspect(docInput, prospectId, patch, options = {}) {
  ...
  const activeFollowups = new Set(["pending", "draft_ready", "approved", "queued", "sending", "failed", "delivery_unknown"]);
  const followups = doc.followups.map(item => {
    ...
    const reason = suppressing ? "Recipient suppressed before delivery." : "Prospect no longer matches the current campaign targeting or recipient.";
    const held = requestUnknownFollowupCancellation(item, reason, options.actorId ?? "admin", now);
    if (held) return held;
    if (item.status === "sending" && item.providerSubmissionStartedAt) return item;
    return {
      ...item,
...
export function createOutreachSponsorInvitation(docInput, prospectId, sponsorPackage, options = {}) {
  ...
  if (doc.followups.some(item => item.prospectId === prospectId && item.kind === "sponsor_outreach" && ["queued", "sending", "delivery_unknown"].includes(item.status))) {
    return { ok: false, error: "A sponsor message is queued or awaiting provider verification. Resolve it before replacing its invitation." };
  }
  const now = options.now ?? new Date().toISOString();
  const nowMs = new Date(now).getTime();
...
export function revokeOutreachSponsorInvitation(docInput, prospectId, options = {}) {
  ...
  if (doc.followups.some(item => item.prospectId === prospectId && item.kind === "sponsor_outreach" && ["queued", "sending", "delivery_unknown"].includes(item.status))) {
    return { ok: false, error: "A sponsor message is queued or awaiting provider verification. Resolve it before revoking its invitation." };
  }
  const now = options.now ?? new Date().toISOString();
  const idFactory = options.idFactory ?? (prefix => `${prefix}_${crypto.randomUUID()}`);
...
export function createSponsorApplicationFromOutreachInvitation(docInput, prospectId, input, options = {}) {
  ...
  const activeFollowups = new Set(["pending", "draft_ready", "approved", "queued", "sending", "failed", "delivery_unknown"]);
  const followups = created.doc.followups.map(item => {
    if (item.prospectId !== prospect.id || item.kind !== "sponsor_outreach" || !activeFollowups.has(item.status)) return item;
    const held = requestUnknownFollowupCancellation(item, "Prospect submitted a sponsor application.", "public", now);
    if (held) return held;
    if (item.status === "sending" && item.providerSubmissionStartedAt) return item;
    return {
      ...item,
      ...
      providerSubmissionStartedAt: null,
      automationJobId: null,
      lastError: "Prospect submitted a sponsor application.",
      updatedAt: now
    };
  });
  const idFactory = options.idFactory ?? (prefix => `${prefix}_${crypto.randomUUID()}`);
...
export function createOutreachCampaign(docInput, input, options = {}) {
  ...
  const idFactory = options.idFactory ?? (prefix => `${prefix}_${crypto.randomUUID()}`);
  const actorId = text(options.actorId, 100) || "admin";
  const campaignId = idFactory("campaign");
  const campaign = {
    id: campaignId,
    name,
  ...
  const existing = doc.campaigns.find(item => item.id === campaignId);
  if (existing) {
    const existingTargeting = existing.targeting || {};
    const sameGeofence = existingTargeting.geofence === null && campaign.targeting.geofence === null
      || existingTargeting.geofence
        && campaign.targeting.geofence
        && existingTargeting.geofence.latitude === campaign.targeting.geofence.latitude
        && existingTargeting.geofence.longitude === campaign.targeting.geofence.longitude
        && existingTargeting.geofence.radiusMiles === campaign.targeting.geofence.radiusMiles;
    const sameTargeting = JSON.stringify(existingTargeting.industries || []) === JSON.stringify(campaign.targeting.industries)
      && JSON.stringify(existingTargeting.cities || []) === JSON.stringify(campaign.targeting.cities)
      && JSON.stringify(existingTargeting.states || []) === JSON.stringify(campaign.targeting.states)
      && JSON.stringify(existingTargeting.postalCodes || []) === JSON.stringify(campaign.targeting.postalCodes)
      && existingTargeting.minFitScore === campaign.targeting.minFitScore
      && sameGeofence;
    const sameSequence = existing.sequence?.length === campaign.sequence.length
      && existing.sequence.every((item, index) => item.id === campaign.sequence[index].id
        && item.order === campaign.sequence[index].order
        && item.delayDays === campaign.sequence[index].delayDays
        && item.subjectTemplate === campaign.sequence[index].subjectTemplate
        && item.bodyTemplate === campaign.sequence[index].bodyTemplate);
    const replay = existing.name === campaign.name
      && existing.objective === campaign.objective
      && sameTargeting
      && sameSequence
      && existing.deliveryMode === campaign.deliveryMode
      && existing.dailySendLimit === campaign.dailySendLimit
      && existing.ownerId === campaign.ownerId
      && existing.createdBy === actorId;
    return replay
      ? { ok: true, replay: true, campaign: existing, doc }
      : { ok: false, code: "IDEMPOTENCY_CONFLICT", error: "This campaign request key was already used for different outreach details." };
  }
  ...
  return {
    ...
    doc: {
      ...
      activity: [...doc.activity, activity(idFactory, "outreach.campaign.created", "campaign", campaign.id, now, actorId)].slice(-1000)
    }
  };
}

function outreachCampaignTarget(campaign) {
  const target = campaign.targeting || {};
  return {
    industries: new Set((target.industries || []).map(item => String(item).toLowerCase())),
    cities: new Set((target.cities || []).map(item => String(item).toLowerCase())),
    states: new Set((target.states || []).map(item => String(item).toLowerCase())),
    postalCodes: new Set((target.postalCodes || []).map(item => String(item).toUpperCase())),
    geofence: target.geofence || null,
    minFitScore: Number(target.minFitScore || 0)
  };
}

function outreachProspectCampaignExclusion(prospect, target) {
  if (prospect.suppressedAt || prospect.status === "do_not_contact") return "suppressed";
  if (!OUTREACH_ELIGIBLE_STATUSES.has(prospect.status)) return "status";
  if (!prospect.contactEmail) return "missing_email";
  if (!prospect.contactBasis) return "missing_contact_basis";
  if (Number(prospect.fitScore || 0) < target.minFitScore) return "below_fit";
  if (target.industries.size && ![...target.industries].some(value => String(prospect.industry || "").toLowerCase().includes(value))) return "industry";
  if (target.cities.size && !target.cities.has(String(prospect.city || "").toLowerCase())) return "city";
  if (target.states.size && !target.states.has(String(prospect.state || "").toLowerCase())) return "state";
  if (target.postalCodes.size) {
    const prospectPostalCode = String(prospect.postalCode || "").toUpperCase();
    if (!target.postalCodes.has(prospectPostalCode) && !target.postalCodes.has(prospectPostalCode.slice(0, 5))) return "postal_code";
  }
  if (target.geofence) {
    const distance = outreachDistanceMiles(prospect.latitude, prospect.longitude, target.geofence.latitude, target.geofence.longitude);
    if (distance === null) return "missing_location";
    if (distance > Number(target.geofence.radiusMiles)) return "outside_radius";
  }
  return null;
}
...
export function matchOutreachProspects(docInput, campaignOrId) {
  ...
  if (!campaign) return [];
  const target = outreachCampaignTarget(campaign);
  return doc.prospects.filter(prospect => outreachProspectCampaignExclusion(prospect, target) === null);
}

export function previewOutreachCampaign(docInput, input, options = {}) {
  const doc = normalizePartnerOperations(docInput);
  const campaignResult = createOutreachCampaign(doc, input, {
    actorId: options.actorId ?? "preview",
    idFactory: () => "campaign_preview",
    now: options.now ?? new Date().toISOString()
  });
  if (!campaignResult.ok) return campaignResult;
  const campaign = campaignResult.campaign;
  const target = outreachCampaignTarget(campaign);
  const exclusions = Object.fromEntries(Object.keys(OUTREACH_CAMPAIGN_EXCLUSION_LABELS).map(reason => [reason, 0]));
  const matches = [];
  for (const prospect of doc.prospects) {
    const exclusion = outreachProspectCampaignExclusion(prospect, target);
    if (exclusion) exclusions[exclusion] += 1;
    else matches.push(prospect);
  }
  const matchLimit = Math.max(1, Math.min(50, Math.round(Number(options.matchLimit) || 25)));
  const publicMatch = prospect => ({
    id: prospect.id,
    ...
    fitScore: Number(prospect.fitScore || 0)
  });
  const sampleProspect = matches[0] || null;
  return {
    ok: true,
    preview: {
      sourceUpdatedAt: doc.lastUpdated || null,
      totalProspects: doc.prospects.length,
      matched: matches.length,
      excluded: doc.prospects.length - matches.length,
      exclusions: Object.entries(exclusions)
        .filter(([, count]) => count > 0)
        .map(([reason, count]) => ({ reason, label: OUTREACH_CAMPAIGN_EXCLUSION_LABELS[reason], count })),
      matches: matches.slice(0, matchLimit).map(publicMatch),
      matchesTruncated: matches.length > matchLimit,
      sample: sampleProspect ? {
        prospect: publicMatch(sampleProspect),
        sequence: campaign.sequence.map(step => ({
          order: step.order,
          delayDays: step.delayDays,
          subject: text(renderOutreachTemplate(step.subjectTemplate, sampleProspect), 180),
          body: text(renderOutreachTemplate(step.bodyTemplate, sampleProspect), 5000)
        }))
      } : null,
...
export function updateOutreachCampaignStatus(docInput, campaignId, action, options = {}) {
  ...
  const inFlightFollowups = doc.followups.filter(item => item.campaignId === campaignId && ["sending", "delivery_unknown"].includes(item.status)).length;
  const activeUnsentStatuses = new Set(["pending", "draft_ready", "approved", "queued", "failed", "delivery_unknown"]);
  const followups = action === "pause" || action === "complete" || action === "archive"
    ? doc.followups.map(item => {
      ...
      if (action === "pause") {
        if (item.status === "delivery_unknown") return item;
      ...
      const held = requestUnknownFollowupCancellation(item, `Campaign ${action === "complete" ? "completed" : "archived"} before delivery.`, options.actorId ?? "admin", now);
      if (held) return held;
      dismissedFollowups += 1;
      return {
        ...item,
...
export function generateDueOutreachFollowups(docInput, options = {}) {
  ...
  const now = options.now ?? new Date().toISOString();
  const nowMs = new Date(now).getTime();
  if (!Number.isFinite(nowMs)) return { ok: false, error: "Outreach follow-up time is invalid." };
  const idFactory = options.idFactory ?? (prefix => `${prefix}_${crypto.randomUUID()}`);
...
export function outreachCampaignMetrics(docInput, campaign) {
  ...
  const prospectIdsFor = predicate => new Set(messages
    .filter(item => item.prospectId && predicate(item))
  ...
  const terminalDeliveryStatuses = new Set(["bounced", "blocked", "complaint", "unsubscribed", "failed"]);
  const enrolledProspectIds = prospectIdsFor(() => true);
  const reachedProspectIds = prospectIdsFor(item => item.status === "sent"
    || Boolean(item.sentAt || item.acceptedAt || item.providerMessageId || item.deliveryStatus));
  const deliveredProspectIds = prospectIdsFor(item => Boolean(item.deliveredAt || item.openedAt || item.clickedAt)
    || deliveredStatuses.has(item.deliveryStatus));
  const openedProspectIds = prospectIdsFor(item => Boolean(item.openedAt || item.clickedAt)
    || openedStatuses.has(item.deliveryStatus));
  const clickedProspectIds = prospectIdsFor(item => Boolean(item.clickedAt) || item.deliveryStatus === "clicked");
  const failedDeliveryProspectIds = prospectIdsFor(item => terminalDeliveryStatuses.has(item.deliveryStatus));
  const sponsorApplicationIds = new Set(doc.applications
    .filter(item => item.type === "sponsor")
    .map(item => item.id));
  const linkedSponsorProspectIds = new Set(doc.applications
    .filter(item => item.type === "sponsor" && item.outreachProspectId)
    .map(item => item.outreachProspectId));
  const applicationProspectIds = new Set(doc.prospects
    .filter(item => reachedProspectIds.has(item.id)
      && (sponsorApplicationIds.has(item.convertedApplicationId) || linkedSponsorProspectIds.has(item.id)))
    .map(item => item.id));
  return {
    matched: matchOutreachProspects(doc, campaign).length,
    drafts: messages.filter(item => item.status === "draft_ready").length,
    approved: messages.filter(item => ["approved", "queued", "sending", "delivery_unknown"].includes(item.status)).length,
    inFlight: messages.filter(item => ["sending", "delivery_unknown"].includes(item.status)).length,
    unknownDeliveryMessages: messages.filter(item => item.status === "delivery_unknown").length,
    ...
    funnel: {
      enrolled: enrolledProspectIds.size,
      reached: reachedProspectIds.size,
      delivered: deliveredProspectIds.size,
      opened: openedProspectIds.size,
      clicked: clickedProspectIds.size,
      applications: applicationProspectIds.size,
      deliveryFailures: failedDeliveryProspectIds.size
    }
  };
}
...
function generateImmediateMilestoneFollowups(doc, milestoneId, options = {}) {
  const result = generateDuePartnerFollowups(doc, {
    ...options,
    milestoneId
  });
  return result.ok ? result : { ok: true, changed: false, generated: [], doc };
}
...
export function createPartnerMilestone(docInput, applicationId, input = {}, options = {}) {
  ...
  const idFactory = options.idFactory ?? (prefix => `${prefix}_${crypto.randomUUID()}`);
  const actorId = text(options.actorId, 100) || "admin";
  const milestoneId = idFactory("milestone");
  const existing = doc.milestones.find(item => item.id === milestoneId);
  if (existing) {
    const replay = existing.applicationId === applicationId
      && existing.source === "custom"
      && existing.label === fields.label
      && existing.dueAt === fields.dueAt
      && existing.status === fields.status
      && existing.assigneeTeam === fields.assigneeTeam
      && existing.reminderLeadDays === fields.reminderLeadDays
      && existing.notes === fields.notes
      && existing.createdBy === actorId;
    return replay
      ? { ok: true, replay: true, milestone: existing, doc }
      : { ok: false, code: "IDEMPOTENCY_CONFLICT", error: "This key-date request key was already used for different schedule details." };
  }
  const item = {
    id: milestoneId,
    applicationId,
  ...
  const baseDoc = {
    ...
    activity: [...doc.activity, activity(idFactory, "milestone.created", "milestone", item.id, now, actorId, { applicationId, dueAt: item.dueAt })].slice(-1000)
  };
  const immediate = generateImmediateMilestoneFollowups(baseDoc, item.id, { ...options, now, idFactory });
  return {
    ok: true,
    replay: false,
    milestone: item,
    generatedFollowups: immediate.generated,
    doc: immediate.doc
  };
}
...
export function updatePartnerMilestone(docInput, milestoneId, input = {}, options = {}) {
  ...
  const dismissed = invalidate
    ...
    : { doc: baseDoc, dismissed: 0 };
  const immediate = item.status === "open"
    ? generateImmediateMilestoneFollowups(dismissed.doc, item.id, { ...options, now, idFactory })
    : { generated: [], doc: dismissed.doc };
  return {
    ok: true,
    milestone: item,
    dismissedFollowups: dismissed.dismissed,
...
export function resolveFollowupRecipient(docInput, followupId, options = {}) {
  ...
  if (followup.taskId) {
    ...
    const expectedSource = followup.kind === "task_assignment"
      ? taskAssignmentSourceVersion(task)
  ...
  if (["application_approved", "application_rejected"].includes(followup.kind)) {
    const expectedDecision = followup.kind === "application_approved" ? "approved" : "rejected";
    const expectedSourceVersion = `decision:${Math.max(0, Math.round(Number(application.decisionVersion) || 0))}:${expectedDecision}`;
    if (application.decisionStatus !== expectedDecision || followup.sourceVersion !== expectedSourceVersion) {
      return { ok: false, error: "The application decision notice is stale after a status change." };
    }
  }
  if (followup.kind === "vendor_applications_open") {
    const opening = currentVendorApplicationOpening(application, options.vendorOfferings);
    if (!opening) return { ok: false, error: "The vendor offering is no longer open for this interest." };
    if (followup.offeringId !== opening.id || followup.sourceVersion !== opening.sourceVersion) {
      return { ok: false, error: "The vendor application opening notice is stale after an offering change." };
    }
    if (!followup.applicationUrl) return { ok: false, error: "The vendor application opening notice is missing its application link." };
    if (typeof options.applicationUrlForInterest === "function"
      && text(options.applicationUrlForInterest(application, opening), 1000) !== followup.applicationUrl) {
      return { ok: false, error: "The vendor application link changed after this notice was created." };
    }
  }
  if (followup.kind === "sponsor_brand_changes" && !followup.brandProfileId && !followup.brandAssetId) {
    return { ok: false, error: "The sponsor brand notice is missing its review target." };
  }
  if (followup.kind === "sponsor_deliverable_review" && !followup.deliverableId) {
    return { ok: false, error: "The sponsor deliverable notice is missing its proof target." };
  }
  if (followup.paymentId) {
    const payment = doc.payments.find(item => item.id === followup.paymentId && item.applicationId === application.id);
    if (!payment) return { ok: false, error: "Payment not found for follow-up." };
    const currentNotice = currentPaymentNotice(payment);
    if (!currentNotice || currentNotice.kind !== followup.kind || currentNotice.sourceVersion !== followup.sourceVersion) {
      return { ok: false, error: "The payment notice is stale after a ledger adjustment." };
    }
  }
  if (followup.brandProfileId) {
    const profile = doc.brandProfiles.find(item => item.id === followup.brandProfileId && item.applicationId === application.id);
    if (!profile || profile.status !== "changes_requested" || followup.kind !== "sponsor_brand_changes") {
      return { ok: false, error: "The sponsor brand profile notice is no longer actionable." };
    }
    if (String(followup.sourceVersion || "") !== `review:${profile.updatedAt}`) {
      return { ok: false, error: "The sponsor brand profile notice is stale after another review." };
    }
  }
  if (followup.brandAssetId) {
    const asset = doc.brandAssets.find(item => item.id === followup.brandAssetId && item.applicationId === application.id);
    const currentAsset = [...doc.brandAssets].reverse().find(item => item.applicationId === application.id
      && item.kind === asset?.kind
      && item.status !== "archived");
    if (!asset || !["changes_requested", "rejected"].includes(asset.status) || currentAsset?.id !== asset.id || followup.kind !== "sponsor_brand_changes") {
      return { ok: false, error: "The sponsor brand asset notice is no longer actionable." };
    }
    if (String(followup.sourceVersion || "") !== `review:${asset.updatedAt}`) {
      return { ok: false, error: "The sponsor brand asset notice is stale after another review." };
    }
  }
  if (followup.deliverableId) {
    const deliverable = doc.deliverables.find(item => item.id === followup.deliverableId && item.applicationId === application.id);
    if (!deliverable || deliverable.status === "cancelled" || deliverable.partnerReviewStatus !== "pending"
      || (!deliverable.proofUrl && !deliverable.proofNotes) || followup.kind !== "sponsor_deliverable_review") {
      return { ok: false, error: "The sponsor deliverable notice is no longer actionable." };
    }
    if (String(followup.sourceVersion || "") !== `proof:${Number(deliverable.proofVersion || 0)}`) {
      return { ok: false, error: "The sponsor deliverable notice is stale after a proof revision." };
    }
  }
...
function outreachCampaignDailyCounts(doc, campaign, now) {
  ...
  const queuedPending = messages.filter(item => ["queued", "sending", "delivery_unknown"].includes(item.status)).length;
  const queuedToday = messages.filter(item => ["queued", "sending", "delivery_unknown"].includes(item.status) && calendarDay(item.queuedAt) === today).length;
...
export function applyTransactionalFollowupAutomation(docInput, options = {}) {
  ...
  const skipped = [];
  const followups = doc.followups.map(item => {
    if (approved.length >= maxBatch || item.status !== "draft_ready" || item.manualReviewRequiredAt || !PARTNER_TRANSACTIONAL_FOLLOWUP_KIND_SET.has(item.kind)) return item;
...
export function applyOutreachCampaignAutomation(docInput, options = {}) {
  ...
  for (const campaign of doc.campaigns.filter(item => item.status === "active" && item.deliveryMode === "approved_sequence")) {
    ...
    const candidates = followups
      ...
      .filter(({ item }) => item.campaignId === campaign.id && item.kind === "sponsor_outreach" && item.status === "draft_ready" && !item.manualReviewRequiredAt)
...
export function prepareFollowupDraft(docInput, followupId, options = {}) {
  ...
  const typeLabel = isVendorInterest ? "vendor interest" : application.type === "vendor" ? "vendor" : "sponsorship";
  const offeringCopy = application.type === "vendor" && application.offeringName
    ? isVendorInterest
      ? ` You selected ${application.offeringName}; fees and availability will be confirmed if applications open.`
  ...
  const portalUrl = text(options.portalUrl, 1000);
  const portalCopy = portalUrl
    ? `\n\n${isVendorInterest ? "Track this submission and next steps" : "Track the application, payment progress, and key dates"} here:\n${portalUrl}`
    : "";
  const followup = {
    ...current,
    status: "draft_ready",
    subject: `Texas SandFest ${typeLabel}${isVendorInterest ? "" : " application"} ${application.reference}`,
    body: `Hello ${application.contactName},\n\nThank you for your interest in Texas SandFest. We received ${application.organizationName}'s ${typeLabel}${isVendorInterest ? "" : " application"} (${application.reference}).${offeringCopy} Our team will review it and follow up with next steps.${portalCopy}\n\nTexas SandFest`,
    portalUrl: portalUrl || null,
    updatedAt: now
  };
  const followups = doc.followups.slice();
...
export function generateDueTaskFollowups(docInput, options = {}) {
  ...
  const activeFollowupStatuses = new Set(["pending", "draft_ready", "approved", "queued", "sending", "failed", "delivery_unknown"]);
  const generated = [];
  ...
  for (const task of doc.tasks) {
    ...
    const portalUrl = typeof options.taskPortalUrlForTask === "function" ? options.taskPortalUrlForTask(task) : null;
    const portalCopy = portalUrl
      ? `\n\nOpen your private task page to acknowledge, start, report a blocker, or complete this assignment:\n${portalUrl}`
    ...
    const assignmentSource = taskAssignmentSourceVersion(task);
    if (!candidates().some(item => item.kind === "task_assignment" && item.sourceVersion === assignmentSource)) {
      ...
      generated.push(build(
        ...
        `Hello ${recipient.name},\n\nYou have been assigned the ${text(task.priority, 20) || "normal"}-priority Texas SandFest task “${task.title}”.${dueCopy}${detailCopy}${portalCopy}\n\nTexas SandFest`
      ));
    }
    ...
    generated.push(build(
      ...
      `Hello ${recipient.name},\n\nThe Texas SandFest task “${task.title}” is ${daysOverdue} day${daysOverdue === 1 ? "" : "s"} overdue. It remains ${text(task.status, 20).replace(/_/g, " ")} and ${text(task.priority, 20) || "normal"} priority.${detailCopy}${portalCopy}\n\nTexas SandFest`,
...
export function generateVendorApplicationOpeningFollowups(docInput, offeringsInput, options = {}) {
  const doc = normalizePartnerOperations(docInput);
  const offerings = Array.isArray(offeringsInput) ? offeringsInput : [];
  const now = options.now ?? new Date().toISOString();
  const idFactory = options.idFactory ?? (prefix => `${prefix}_${crypto.randomUUID()}`);
  const staleStatuses = new Set(["pending", "draft_ready", "approved", "queued", "failed"]);
  let dismissedFollowups = 0;
  const followups = doc.followups.map(item => {
    if (item.kind !== "vendor_applications_open" || !staleStatuses.has(item.status)) return item;
    const application = doc.applications.find(candidate => candidate.id === item.applicationId);
    const opening = currentVendorApplicationOpening(application, offerings);
    if (opening && item.offeringId === opening.id && item.sourceVersion === opening.sourceVersion) return item;
    dismissedFollowups += 1;
    return {
      ...item,
      status: "dismissed",
      ...
      approvedAt: null,
      automationPolicy: null,
      automationDecision: "vendor_offering_changed",
      automationApprovedAt: null,
      automationJobId: null,
      dismissedAt: now,
      dismissedBy: "automation",
      lastError: "Vendor offering changed before delivery.",
      updatedAt: now
    };
  });
  const generated = [];
  ...
  for (const application of doc.applications) {
    const opening = currentVendorApplicationOpening(application, offerings);
    const recipient = email(application.contactEmail);
    if (!opening || application.consentToContact !== true || !recipient) continue;
    if (followups.some(item => item.kind === "vendor_applications_open"
      && item.applicationId === application.id
      && item.status === "delivery_unknown")) continue;
    if ([...followups, ...generated].some(item => item.kind === "vendor_applications_open"
      && item.applicationId === application.id
      && item.sourceVersion === opening.sourceVersion)) continue;
    const applicationUrl = typeof options.applicationUrlForInterest === "function"
      ? text(options.applicationUrlForInterest(application, opening), 1000)
      : "";
    if (!applicationUrl) continue;
    generated.push({
      id: idFactory("followup"),
      applicationId: application.id,
      offeringId: opening.id,
      kind: "vendor_applications_open",
      workflowKey: `vendor-opening:${application.id}:${opening.id}`,
      sourceVersion: opening.sourceVersion,
      channel: "email",
      recipient,
      status: "draft_ready",
      dueAt: now,
      subject: `Texas SandFest vendor applications are open - ${opening.name}`,
      body: `Hello ${application.contactName},\n\nTexas SandFest vendor applications are now open for ${opening.name} (${opening.publicLabel}). Your earlier interest remains non-financial and has not been converted into an application. To apply, review the current terms and submit a new application with your current details and consent:\n${applicationUrl}\n\nTexas SandFest`,
      applicationUrl,
      approvedBy: null,
      approvedAt: null,
      sentAt: null,
  ...
  if (!generated.length && !dismissedFollowups) {
    return { ok: true, changed: false, generated: [], dismissedFollowups: 0, doc };
  }
  const activityRecords = generated.map(item => activity(
    idFactory,
    "vendor.opening_followup.generated",
    "followup",
    item.id,
    now,
    "automation",
    { applicationId: item.applicationId, offeringId: item.offeringId, sourceVersion: item.sourceVersion }
  ));
  return {
    ok: true,
    changed: true,
    ...
    doc: {
      ...doc,
      lastUpdated: now,
      followups: [...followups, ...generated],
      activity: [...doc.activity, ...activityRecords].slice(-1000)
    }
  };
}

export function generatePartnerPaymentFollowups(docInput, options = {}) {
  const doc = normalizePartnerOperations(docInput);
  const now = options.now ?? new Date().toISOString();
  const nowMs = new Date(now).getTime();
  if (!Number.isFinite(nowMs)) return { ok: false, error: "Payment follow-up time is invalid." };
  const idFactory = options.idFactory ?? (prefix => `${prefix}_${crypto.randomUUID()}`);
  const staleStatuses = new Set(["pending", "draft_ready", "approved", "failed"]);
  let dismissedFollowups = 0;
  const followups = doc.followups.map(item => {
    if (!item.paymentId || !["payment_received", "payment_adjustment"].includes(item.kind) || !staleStatuses.has(item.status)) return item;
    const payment = doc.payments.find(candidate => candidate.id === item.paymentId);
    const current = currentPaymentNotice(payment);
    if (current?.kind === item.kind && current.sourceVersion === item.sourceVersion) return item;
    dismissedFollowups += 1;
    return {
      ...item,
      status: "dismissed",
      ...
      approvedAt: null,
      automationPolicy: null,
      automationDecision: "payment_state_changed",
      automationApprovedAt: null,
      automationJobId: null,
      dismissedAt: now,
      dismissedBy: "automation",
      lastError: "Payment state changed before delivery.",
      updatedAt: now
    };
  });
  const generated = [];
  ...
  for (const payment of doc.payments) {
    const notice = currentPaymentNotice(payment);
    if (!notice) continue;
    if (followups.some(item => item.paymentId === payment.id && item.status === "delivery_unknown")) continue;
    if ([...followups, ...generated].some(item => item.paymentId === payment.id && item.sourceVersion === notice.sourceVersion)) continue;
    const application = doc.applications.find(item => item.id === payment.applicationId);
    const recipient = email(application?.contactEmail);
    if (!application || application.consentToContact !== true || !recipient) continue;
    const invoice = payment.invoiceId
      ? doc.invoices.find(item => item.id === payment.invoiceId && item.applicationId === application.id) || null
      : null;
    const balanceCents = invoice ? localInvoiceBalance(doc, invoice) : null;
    const portalUrl = typeof options.portalUrlForApplication === "function"
      ? text(options.portalUrlForApplication(application), 1000)
      : "";
    const portalCopy = portalUrl ? `\n\nView the current balance, key dates, and payment history in the private partner portal:\n${portalUrl}` : "";
    const receivedAt = iso(payment.receivedAt || payment.createdAt, now);
    const receivedLabel = new Date(receivedAt).toLocaleDateString("en-US", { timeZone: "America/Chicago" });
    const balanceCopy = balanceCents === null
      ? " This payment is not allocated to an invoice yet."
      : balanceCents > 0
        ? ` The current invoice balance is ${money(balanceCents)}.`
        : " The approved invoice balance is paid in full.";
    const refundedAmountCents = cents(payment.refundedAmountCents);
    const adjustmentCopy = payment.status === "voided"
      ? `The ${money(payment.amountCents)} payment entry dated ${receivedLabel} was voided.${balanceCopy}`
      : `${money(refundedAmountCents)} of the ${money(payment.amountCents)} payment dated ${receivedLabel} has been refunded.${balanceCopy}`;
    const subject = notice.kind === "payment_received"
      ? `Texas SandFest payment received - ${application.reference}`
      : `Texas SandFest payment adjustment - ${application.reference}`;
    const body = notice.kind === "payment_received"
      ? `Hello ${application.contactName},\n\nWe recorded a payment of ${money(payment.amountCents)} for ${application.organizationName}'s Texas SandFest ${application.type} application (${application.reference}) on ${receivedLabel}.${balanceCopy}${portalCopy}\n\nTexas SandFest`
      : `Hello ${application.contactName},\n\nWe recorded a payment adjustment for ${application.organizationName}'s Texas SandFest ${application.type} application (${application.reference}). ${adjustmentCopy}${portalCopy}\n\nTexas SandFest`;
    generated.push({
      id: idFactory("followup"),
      applicationId: application.id,
      ...
      kind: notice.kind,
      sourceVersion: notice.sourceVersion,
      channel: "email",
      recipient,
      status: "draft_ready",
      dueAt: now,
  ...
  const activityRecords = generated.map(item => activity(
    ...
    now,
    "automation",
    { applicationId: item.applicationId, paymentId: item.paymentId, kind: item.kind, sourceVersion: item.sourceVersion }
  ));
  return {
    ok: true,
    changed: true,
...
export function generateDuePartnerFollowups(docInput, options = {}) {
  ...
  const now = options.now ?? new Date().toISOString();
  const nowMs = new Date(now).getTime();
  if (!Number.isFinite(nowMs)) return { ok: false, error: "Partner follow-up time is invalid." };
  ...
  for (const due of doc.milestones) {
    if (options.milestoneId && due.id !== options.milestoneId) continue;
    ...
    const applicationCreatedMs = new Date(application.createdAt || "").getTime();
    const initialIntakeReminderIsTooEarly = reminder.phase === "upcoming"
      && due.source === "application_intake"
      && Number(due.scheduleVersion || 1) === 1
      && INITIAL_INTAKE_MILESTONE_KINDS.has(due.kind)
      && Number.isFinite(applicationCreatedMs)
      && nowMs - applicationCreatedMs < PARTNER_INITIAL_REMINDER_GRACE_MS;
    if (initialIntakeReminderIsTooEarly) continue;
...
export function reviewFollowup(docInput, followupId, action, options = {}) {
  ...
  if (index < 0) return { ok: false, error: "Follow-up not found." };
  const current = doc.followups[index];
  if (current.status === "delivery_unknown" || current.deliveryOutcomeUnknown) {
    return { ok: false, conflict: true, outcomeUnknown: true, error: "Verify the provider outcome before reviewing this message." };
  }
  const now = options.now ?? new Date().toISOString();
  if (action === "approve") {
...
const PROTECTED_FOLLOWUP_URL_FIELDS = ["portalUrl", "applicationUrl", "sponsorInvitationUrl"];
const PROTECTED_FOLLOWUP_HASH = /#(?:partner-status|task-status|outreach-preferences|sponsor-invitation)\?/;

function protectedFollowupUrls(followup) {
  const urls = new Set(PROTECTED_FOLLOWUP_URL_FIELDS.map(field => text(followup?.[field], 1000)).filter(Boolean));
  for (const match of String(followup?.body || "").matchAll(/https?:\/\/[^\s<>"']+/g)) {
    if (PROTECTED_FOLLOWUP_HASH.test(match[0])) urls.add(match[0]);
  }
  return [...urls];
}

export function editFollowupDraft(docInput, followupId, input = {}, options = {}) {
  const doc = normalizePartnerOperations(docInput);
  const index = doc.followups.findIndex(item => item.id === followupId);
  if (index < 0) return { ok: false, error: "Follow-up not found." };
  const current = doc.followups[index];
  if (current.status !== "draft_ready") return { ok: false, conflict: true, error: "Only ready drafts can be edited." };
  const expectedUpdatedAt = text(input.expectedUpdatedAt, 50);
  if (!expectedUpdatedAt || expectedUpdatedAt !== current.updatedAt) {
    return { ok: false, conflict: true, error: "This draft changed after it was opened. Refresh it before editing." };
  }
  const subject = String(input.subject ?? "").trim();
  const body = String(input.body ?? "").replace(/\r\n?/g, "\n").trim();
  if (!subject) return { ok: false, error: "Message subject is required." };
  if (subject.length > 180) return { ok: false, error: "Message subject must be 180 characters or fewer." };
  if (/\r|\n/.test(subject) || /[\u0000-\u0008\u000B\u000C\u000E-\u001F\u007F]/.test(subject)) {
    return { ok: false, error: "Message subject contains unsupported characters." };
  }
  if (!body) return { ok: false, error: "Message body is required." };
  if (body.length > 7500) return { ok: false, error: "Message body must be 7,500 characters or fewer." };
  if (/[\u0000-\u0008\u000B\u000C\u000E-\u001F\u007F]/.test(body)) {
    return { ok: false, error: "Message body contains unsupported characters." };
  }
  if (protectedFollowupUrls(current).some(url => !body.includes(url))) {
    return { ok: false, error: "Keep every private action and outreach-preference link in the message." };
  }
  const subjectChanged = subject !== current.subject;
  const bodyChanged = body !== current.body;
  if (!subjectChanged && !bodyChanged) return { ok: true, changed: false, previous: current, followup: current, doc };
  const now = options.now ?? new Date().toISOString();
  const actorId = options.actorId ?? "admin";
  const idFactory = options.idFactory ?? (prefix => `${prefix}_${crypto.randomUUID()}`);
  const followup = {
    ...
    subject,
    body,
    editVersion: Math.max(0, Math.round(Number(current.editVersion) || 0)) + 1,
    editedBy: actorId,
    editedAt: now,
  ...
  const followups = doc.followups.slice();
  followups[index] = followup;
  const change = { subjectChanged, bodyChanged, subjectLength: subject.length, bodyLength: body.length, editVersion: followup.editVersion };
  return {
    ok: true,
    changed: true,
    change,
    ...
    doc: {
      ...
      lastUpdated: now,
      followups,
      activity: [...doc.activity, activity(idFactory, "followup.edited", "followup", followupId, now, actorId, change)].slice(-1000)
    }
  };
}

export function releaseAutomatedFollowupApproval(docInput, followupId, reason, options = {}) {
  const doc = normalizePartnerOperations(docInput);
  const index = doc.followups.findIndex(item => item.id === followupId);
  if (index < 0) return { ok: false, error: "Follow-up not found." };
  const current = doc.followups[index];
  const automationPolicy = text(options.automationPolicy || current.automationPolicy, 100);
  if (current.status !== "approved") return { ok: false, error: "Follow-up is not awaiting automated delivery." };
  if (![PARTNER_TRANSACTIONAL_AUTOMATION_POLICY, OUTREACH_CAMPAIGN_AUTOMATION_POLICY].includes(automationPolicy)
    || current.automationPolicy !== automationPolicy
    || current.approvedBy !== `automation:${automationPolicy}`) {
    return { ok: false, error: "Follow-up does not carry the matching automation approval." };
  }
  const now = options.now ?? new Date().toISOString();
  const idFactory = options.idFactory ?? (prefix => `${prefix}_${crypto.randomUUID()}`);
  const lastError = text(reason, 1000) || "Automated delivery could not be queued; approval was released for a future run.";
  const followup = {
    ...current,
    status: "draft_ready",
    approvedBy: null,
    approvedAt: null,
    automationPolicy: null,
    automationDecision: text(options.decision, 100) || "queue_rejected",
    automationApprovedAt: null,
    automationCampaignApprovedAt: null,
    automationJobId: null,
  ...
  return {
    ...
    doc: {
      ...
      activity: [...doc.activity, activity(
        idFactory,
        "followup.automation_released",
        "followup",
        followupId,
        now,
        options.actorId ?? "worker",
        { automationPolicy, reason: followup.automationDecision }
      )].slice(-1000)
    }
  };
}
...
export function queueFollowupDelivery(docInput, followupId, options = {}) {
  ...
  const current = doc.followups[index];
  if (current.status === "delivery_unknown" || current.deliveryOutcomeUnknown) {
    return { ok: false, conflict: true, outcomeUnknown: true, error: "Verify the provider outcome before queueing this message again." };
  }
...
export function claimFollowupDelivery(docInput, followupId, options = {}) {
  ...
  if (current.status !== "queued") {
    return { ok: false, canceled: ["draft_ready", "dismissed", "failed", "delivery_unknown"].includes(current.status), status: current.status, error: `Follow-up is ${current.status}, not queued.` };
  }
...
export function beginFollowupProviderSubmission(docInput, followupId, options = {}) {
  ...
  if (current.status !== "sending") {
    return { ok: false, canceled: ["draft_ready", "dismissed", "failed", "delivery_unknown"].includes(current.status), status: current.status, error: `Follow-up is ${current.status}, not sending.` };
  }
  const deliveryClaimId = text(options.deliveryClaimId, 100);
...
export function recordFollowupDelivery(docInput, followupId, delivery, options = {}) {
  ...
  const outcomeUnknown = options.unknownOutcome === true;
  ...
  const followup = {
    ...current,
    status: outcomeUnknown ? "delivery_unknown" : sent ? "sent" : terminal ? "failed" : "queued",
    ...
    deliveryOutcomeUnknown: outcomeUnknown,
    deliveryResolution: null,
    deliveryReconciledAt: null,
    deliveryReconciledBy: null,
    deliveryResolutionNote: null,
...
export function reconcileFollowupDelivery(docInput, followupId, input, options = {}) {
  const doc = normalizePartnerOperations(docInput);
  const index = doc.followups.findIndex(item => item.id === followupId);
  if (index < 0) return { ok: false, error: "Follow-up not found." };
  const current = doc.followups[index];
  if (current.status !== "delivery_unknown" || !current.deliveryOutcomeUnknown) {
    return { ok: false, conflict: true, error: "This message does not have an unknown provider outcome." };
  }
  const action = FOLLOWUP_DELIVERY_RESOLUTIONS.has(input?.action) ? input.action : null;
  if (!action) return { ok: false, error: "Action must be confirmed_sent or confirmed_not_sent." };
  const resolutionNote = text(input?.resolutionNote, 1000);
  if (resolutionNote.length < 12) return { ok: false, error: "A verification note of at least 12 characters is required." };
  const providerMessageId = text(input?.providerMessageId, 500);
  if (action === "confirmed_sent" && !providerMessageId) {
    return { ok: false, error: "The provider message ID is required to confirm delivery." };
  }
  const now = options.now ?? new Date().toISOString();
  const actorId = text(options.actorId, 120) || "admin";
  const sent = action === "confirmed_sent";
  const canceled = !sent && Boolean(current.deliveryCancellationRequestedAt);
  const provider = current.provider && current.provider !== "worker" ? current.provider : "brevo";
  const followup = normalizePartnerFollowup({
    ...current,
    status: sent ? "sent" : canceled ? "dismissed" : "failed",
    provider,
    providerMessageId: sent ? providerMessageId : null,
    deliveryStatus: sent ? "accepted" : null,
    acceptedAt: sent ? now : null,
    sentAt: sent ? now : null,
    dismissedAt: canceled ? now : current.dismissedAt ?? null,
    dismissedBy: canceled ? current.deliveryCancellationRequestedBy || actorId : current.dismissedBy ?? null,
    deliveryClaimId: null,
    deliveryClaimedAt: null,
    providerSubmissionStartedAt: sent ? current.providerSubmissionStartedAt : null,
    deliveryOutcomeUnknown: false,
    deliveryResolution: action,
    deliveryReconciledAt: now,
    deliveryReconciledBy: actorId,
    deliveryResolutionNote: resolutionNote,
    lastError: sent
      ? null
      : canceled
        ? current.deliveryCancellationReason || "Provider confirmed no delivery; the message is no longer needed."
        : text(`Provider confirmed no delivery. ${resolutionNote}`, 1000),
    updatedAt: now
  });
  let followups = doc.followups.slice();
  followups[index] = followup;
  ...
  return {
    ...
    doc: {
      ...
      activity: [...doc.activity, activity(
        idFactory,
        "followup.delivery_reconciled",
        "followup",
        followupId,
        now,
        actorId,
        { resolution: action, canceled }
      )].slice(-1000)
    }
  };
}
...
export function summarizeVendorReadiness(docInput, now = new Date().toISOString()) {
  const doc = normalizePartnerOperations(docInput);
  const interests = doc.applications.filter(item => item.type === "vendor" && item.intakeMode === "interest");
  const vendors = doc.applications.filter(item => item.type === "vendor" && item.intakeMode !== "interest").map(application => {
...
export function summarizePartnerOperations(docInput, now = new Date().toISOString()) {
  ...
  return {
    ...
    operations: {
      ...
      unknownDeliveryMessages: doc.followups.filter(item => item.status === "delivery_unknown").length
    },
    fulfillment,
    ...
    outreach: {
      ...
      unknownDeliveryMessages: doc.followups.filter(item => item.kind === "sponsor_outreach" && item.status === "delivery_unknown").length,
