...
const FOLLOWUP_STATUSES = new Set(["pending", "draft_ready", "approved", "queued", "sent", "dismissed", "failed"]);
...
export function rotatePartnerPortalAccess(docInput, applicationId, options = {}) {
  ...
  const followups = doc.followups.map(item => {
    if (item.applicationId !== applicationId || !portalUrl || !item.portalUrl || ["sent", "dismissed"].includes(item.status)) return item;
...
const ACTIVE_WORKFLOW_FOLLOWUP_STATUSES = new Set(["pending", "draft_ready", "approved", "queued", "failed"]);
...
function upsertVendorWorkflowFollowup(doc, application, input, options = {}) {
  ...
  const followups = doc.followups.map(item => item.workflowKey === workflowKey && item.status === "queued"
    ? { ...item, status: "dismissed", approvedBy: null, approvedAt: null, lastError: "Workflow changed before delivery; review the replacement draft.", updatedAt: now }
    : item);
  let reusableIndex = -1;
...
export function createOutreachSponsorInvitation(docInput, prospectId, sponsorPackage, options = {}) {
  ...
  if (doc.followups.some(item => item.prospectId === prospectId && item.kind === "sponsor_outreach" && item.status === "queued")) {
...
export function updateOutreachCampaignStatus(docInput, campaignId, action, options = {}) {
  ...
  return {
    ...
    doc: {
      ...
      campaigns,
      followups,
      activity: [...doc.activity, activity(idFactory, `outreach.campaign.${action}`, "campaign", campaignId, now, options.actorId ?? "admin", { returnedToReview, dismissedFollowups })].slice(-1000)
    }
  };
}
...
export function outreachCampaignMetrics(docInput, campaign) {
  ...
  return {
    ...
    approved: messages.filter(item => ["approved", "queued"].includes(item.status)).length,
...
function outreachCampaignDailyCounts(doc, campaign, now) {
  ...
  const messages = doc.followups.filter(item => item.campaignId === campaign.id && item.automationPolicy === OUTREACH_CAMPAIGN_AUTOMATION_POLICY);
  const sentToday = messages.filter(item => item.status === "sent" && calendarDay(item.sentAt) === today).length;
  const queuedPending = messages.filter(item => item.status === "queued").length;
  const queuedToday = messages.filter(item => item.status === "queued" && calendarDay(item.queuedAt) === today).length;
  const approvedPending = messages.filter(item => item.status === "approved" && outreachAutomatedApproval(item, campaign)).length;
  return { sentToday, queuedPending, queuedToday, approvedPending };
}
...
export function outreachCampaignAutomationReadiness(docInput, campaignOrId, options = {}) {
  ...
  const sendSlotsToday = Math.max(0, campaign.dailySendLimit - counts.sentToday - counts.queuedPending);
  return {
...
export function recordFollowupDelivery(docInput, followupId, delivery, options = {}) {
  ...
  if (index < 0) return { ok: false, error: "Follow-up not found." };
  const current = doc.followups[index];
  if (current.status !== "queued") return { ok: false, error: "Follow-up is not queued for delivery." };
  const now = options.now ?? new Date().toISOString();
