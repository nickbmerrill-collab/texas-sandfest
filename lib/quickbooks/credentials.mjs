...
function instantMs(value = Date.now()) {
...
function nowIso(value = Date.now()) {
  ...
  if (!Number.isFinite(timestamp)) throw new Error("QuickBooks credential clock is invalid.");
...
export async function beginQuickBooksAuthorization(root, { actorId, now = Date.now() } = {}, env = process.env) {
  ...
  if (!Number.isFinite(nowMs)) return { ok: false, error: "QuickBooks credential clock is invalid.", quickbooks: await readQuickBooksCredentialStatus(root, env) };
  const state = randomBytes(32).toString("base64url");
  const createdAt = nowIso(nowMs);
  const expiresAt = nowIso(nowMs + OAUTH_ATTEMPT_TTL_MS);
...
async function consumeAuthorizationAttempt(root, state, now, env) {
  ...
  await updatePlatformDoc(root, "quickBooksCredentials", current => {
    ...
    return { ...doc, pendingAttempts, lastUpdated: nowIso(nowMs) };
  }, { fallback: emptyQuickBooksCredentialStore(env) });
  return consumed;
}
...
function tokenExpiry(now, seconds) {
  ...
  return Number.isFinite(value) && value > 0 ? nowIso(nowMs + Math.round(value * 1000)) : null;
}
...
export async function completeQuickBooksAuthorization(root, { state, code, realmId, now = Date.now(), fetchImpl } = {}, env = process.env) {
  ...
  const nowMs = instantMs(now);
  if (!Number.isFinite(nowMs)) return { ok: false, error: "QuickBooks credential clock is invalid." };
  ...
  const attempt = await consumeAuthorizationAttempt(root, state, nowMs, env);
  ...
  const connectedAt = nowIso(nowMs);
  await updatePlatformDoc(root, "quickBooksCredentials", current => {
    const doc = normalizeQuickBooksCredentialStore(current, env);
    ...
    return {
      ...
      connection: {
        ...
        refreshTokenExpiresAt: tokenExpiry(nowMs, token.x_refresh_token_expires_in)
      },
...
export async function persistQuickBooksTokenRotation(root, runtime, token, { now = Date.now() } = {}, env = process.env) {
  ...
  const nowMs = instantMs(now);
  if (!Number.isFinite(nowMs)) return { changed: false };
  let changed = false;
  await updatePlatformDoc(root, "quickBooksCredentials", current => {
    const doc = normalizeQuickBooksCredentialStore(current, env);
    const connection = doc.connection;
    ...
    return {
      ...
      connection: {
        ...
        refreshTokenExpiresAt: tokenExpiry(nowMs, token.x_refresh_token_expires_in) || connection.refreshTokenExpiresAt || null
      },
...
export async function disconnectQuickBooks(root, { now = Date.now() } = {}, env = process.env) {
  const nowMs = instantMs(now);
  if (!Number.isFinite(nowMs)) return { changed: false, quickbooks: await readQuickBooksCredentialStatus(root, env) };
  let changed = false;
  await updatePlatformDoc(root, "quickBooksCredentials", current => {
    const doc = normalizeQuickBooksCredentialStore(current, env);
    changed = Boolean(doc.connection);
    return { ...doc, connection: null, pendingAttempts: [], lastUpdated: nowIso(nowMs) };
  }, { fallback: emptyQuickBooksCredentialStore(env) });
