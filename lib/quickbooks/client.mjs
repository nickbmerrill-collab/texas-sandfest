...
export function quickBooksConfig(env = process.env) {
  ...
  return {
    ...
    authBaseUrl: env.QB_AUTH_BASE_URL || "https://appcenter.intuit.com/connect/oauth2",
    tokenUrl: env.QB_TOKEN_URL || "https://oauth.platform.intuit.com/oauth2/v1/tokens/bearer",
    apiBaseUrl: env.QB_API_BASE_URL || (environment === "production"
      ? "https://quickbooks.api.intuit.com"
      : "https://sandbox-quickbooks.api.intuit.com")
  };
}

export function quickBooksReadiness(env = process.env, { connection = null } = {}) {
  const config = quickBooksConfig(env);
  const required = ["clientId", "clientSecret", "redirectUri"];
  const missing = required.filter(key => !config[key]);
  const canBuildAuthUrl = missing.length === 0;
  const storedCredential = connection?.connected === true;
  const canRefreshToken = canBuildAuthUrl && (storedCredential || Boolean(config.refreshToken));
  const canCallAccountingApi = canRefreshToken && (connection?.realmIdPresent === true || Boolean(config.realmId));
  ...
  return {
    ...
    credentialSource: storedCredential ? "encrypted_store" : config.refreshToken ? "environment" : null,
    missing,
    reason: canSyncPartnerInvoices
      ? null
      : !config.invoiceSyncEnabled
        ? "QuickBooks invoice sync is disabled."
        : `QuickBooks is missing ${[...missing, ...(!storedCredential && !config.refreshToken ? ["refreshToken"] : []), ...(!connection?.realmIdPresent && !config.realmId ? ["realmId"] : [])].join(", ")}.`
  };
}
...
export async function syncPartnerInvoiceToQuickBooks({ application, invoice }, options = {}, env = process.env) {
  ...
  const readiness = quickBooksReadiness(runtimeEnv);
  ...
  const token = await refreshAccessToken(runtimeEnv, { fetchImpl });
  const accessToken = token.access_token;
  if (!accessToken) throw new Error("QuickBooks token refresh returned no access token");
  if (options.onTokenRefresh) await options.onTokenRefresh(token);
...
export async function reconcilePartnerInvoiceFromQuickBooks({ invoice }, options = {}, env = process.env) {
  ...
  if (!accessToken) throw new Error("QuickBooks token refresh returned no access token");
  if (options.onTokenRefresh) await options.onTokenRefresh(token);
  const response = await quickBooksRead("invoice", invoice.quickBooksInvoiceId, { accessToken, fetchImpl }, runtimeEnv);
