...
export const INCOMING_DOCUMENT_EXTRACTION_STATUSES = Object.freeze([
  "stored",
  "preview_ready",
  "queued",
  "extracting",
...
const EXTRACTION_STATUS_SET = new Set(INCOMING_DOCUMENT_EXTRACTION_STATUSES);
...
function textPreview(buffer, rule) {
  if (rule.kind === "extractable") return { extractionStatus: "queued", textPreview: null, previewTruncated: false };
...
export function incomingDocumentExtractionSupported(input) {
  const contentType = typeof input === "string" ? input : input?.contentType;
  return EXTRACTABLE_CONTENT_TYPES.has(String(contentType || "").toLowerCase());
}
...
function normalizeRecord(record) {
  ...
  const extractionStatus = EXTRACTION_STATUS_SET.has(record?.extractionStatus)
    ? record.extractionStatus
    : record?.textPreview
      ? "preview_ready"
      : "stored";
  const extractionChunks = (Array.isArray(record?.extractionChunks) ? record.extractionChunks : [])
    .slice(0, 200)
    .map(chunk => ({
      text: text(chunk?.text, 2_000),
      metadata: {
        pageNumber: Number.isInteger(Number(chunk?.metadata?.pageNumber)) ? Number(chunk.metadata.pageNumber) : null,
        slideNumber: Number.isInteger(Number(chunk?.metadata?.slideNumber)) ? Number(chunk.metadata.slideNumber) : null,
        sheetName: text(chunk?.metadata?.sheetName, 160) || null,
        closestHeading: text(chunk?.metadata?.closestHeading, 240) || null,
        isTableChunk: chunk?.metadata?.isTableChunk === true
      }
    }))
  ...
  return {
    ...
    extractionStatus,
    extractionVersion: Math.max(0, Math.round(Number(record?.extractionVersion) || 0)),
    extractionAttempts: Math.max(0, Math.round(Number(record?.extractionAttempts) || 0)),
    extractionRequestedAt: iso(record?.extractionRequestedAt),
    extractionRequestedBy: text(record?.extractionRequestedBy, 180) || null,
    extractionStartedAt: iso(record?.extractionStartedAt),
    extractionCompletedAt: iso(record?.extractionCompletedAt),
    extractionJobId: text(record?.extractionJobId, 180) || null,
    extractionError: text(record?.extractionError, 1_000) || null,
    extractionWarnings: (Array.isArray(record?.extractionWarnings) ? record.extractionWarnings : []).map(item => text(item, 500)).filter(Boolean).slice(0, 20),
    extractedCharacterCount: Math.max(0, Math.round(Number(record?.extractedCharacterCount) || 0)),
    extractedChunkCount: Math.max(0, Math.round(Number(record?.extractedChunkCount) || extractionChunks.length)),
    extractionTruncated: record?.extractionTruncated === true,
    extractionChunks,
...
export function createIncomingDocument(inputDoc, input, options = {}) {
  ...
  const record = normalizeRecord({
    ...
    status: "received",
    extractionVersion: input?.extractionStatus === "queued" ? 1 : 0,
    extractionAttempts: 0,
    extractionRequestedAt: input?.extractionStatus === "queued" ? now : null,
    extractionRequestedBy: input?.extractionStatus === "queued" ? text(options.actorId, 180) || "unknown" : null,
    notes: null,
    uploadedAt: now,
...
function replaceIncomingDocument(doc, index, record, now) {
  const documents = doc.documents.slice();
  documents[index] = normalizeRecord(record);
  return { ...doc, lastUpdated: now, documents };
}

export function requestIncomingDocumentExtraction(inputDoc, documentId, options = {}) {
  const eventId = text(options.eventId, 80) || DEFAULT_EVENT_ID;
  const doc = normalizeIncomingDocumentIntake(inputDoc, { eventId });
  ...
  if (!incomingDocumentExtractionSupported(current)) return { ok: false, unsupported: true, error: "This document type does not support automatic text extraction.", doc };
  if (!options.force && ["queued", "extracting"].includes(current.extractionStatus)) {
    return { ok: true, changed: false, document: current, doc };
  }
  const now = iso(options.now) || new Date().toISOString();
  const extractionVersion = Math.max(1, current.extractionVersion + (options.force ? 1 : 0));
  const document = normalizeRecord({
    ...current,
    extractionStatus: "queued",
    extractionVersion,
    extractionRequestedAt: now,
    extractionRequestedBy: text(options.actorId, 180) || "unknown",
  ...
  return { ok: true, changed: true, document, before: current, doc: replaceIncomingDocument(doc, index, document, now) };
}

export function beginIncomingDocumentExtraction(inputDoc, documentId, input = {}, options = {}) {
  const eventId = text(options.eventId, 80) || DEFAULT_EVENT_ID;
  const doc = normalizeIncomingDocumentIntake(inputDoc, { eventId });
  const index = doc.documents.findIndex(record => record.id === documentId);
  if (index < 0) return { ok: false, error: "Document not found.", doc };
  const current = doc.documents[index];
  const extractionVersion = Math.max(0, Math.round(Number(input.extractionVersion) || 0));
  if (current.extractionVersion !== extractionVersion) return { ok: false, stale: true, error: "Document extraction request is stale.", doc };
  if (!["queued", "extracting"].includes(current.extractionStatus)) {
    return { ok: false, canceled: true, status: current.extractionStatus, error: `Document extraction is ${current.extractionStatus}.`, doc };
  }
  const now = iso(options.now) || new Date().toISOString();
  const jobId = text(input.jobId, 180);
  if (current.extractionStatus === "extracting" && current.extractionJobId && current.extractionJobId !== jobId) {
    return { ok: false, stale: true, error: "Another extraction job owns this document.", doc };
  }
  const document = normalizeRecord({
    ...current,
    extractionStatus: "extracting",
    extractionAttempts: current.extractionAttempts + (current.extractionStatus === "queued" ? 1 : 0),
    extractionStartedAt: current.extractionStartedAt || now,
    extractionJobId: jobId || current.extractionJobId,
    extractionError: null,
    updatedAt: now
  });
  return { ok: true, document, doc: replaceIncomingDocument(doc, index, document, now) };
}

export function completeIncomingDocumentExtraction(inputDoc, documentId, input = {}, options = {}) {
  const eventId = text(options.eventId, 80) || DEFAULT_EVENT_ID;
  const doc = normalizeIncomingDocumentIntake(inputDoc, { eventId });
  const index = doc.documents.findIndex(record => record.id === documentId);
  if (index < 0) return { ok: false, error: "Document not found.", doc };
  ...
  const extractionVersion = Math.max(0, Math.round(Number(input.extractionVersion) || 0));
  if (current.extractionVersion !== extractionVersion) return { ok: false, stale: true, error: "Document extraction request is stale.", doc };
  if (!["queued", "extracting"].includes(current.extractionStatus)) return { ok: false, canceled: true, status: current.extractionStatus, error: `Document extraction is ${current.extractionStatus}.`, doc };
  const now = iso(options.now) || new Date().toISOString();
  const normalizedText = String(input.text || "").replace(/\r\n?/g, "\n").trim();
  const extractionChunks = (Array.isArray(input.chunks) ? input.chunks : []).slice(0, 200);
  const hasText = Boolean(normalizedText || extractionChunks.some(chunk => String(chunk?.text || "").trim()));
  const document = normalizeRecord({
    ...current,
    extractionStatus: hasText ? "ready" : "needs_review",
    extractionCompletedAt: now,
    extractionError: hasText ? null : "No machine-readable text was found. Review the source or provide an OCR-approved copy.",
    extractionWarnings: input.warnings,
    extractedCharacterCount: Math.max(0, Math.round(Number(input.characterCount) || normalizedText.length)),
    extractedChunkCount: Math.max(0, Math.round(Number(input.chunkCount) || extractionChunks.length)),
    extractionTruncated: input.truncated === true,
    extractionChunks,
    textPreview: normalizedText.slice(0, 2_000) || null,
    previewTruncated: normalizedText.length > 2_000 || input.truncated === true,
    updatedAt: now
  });
  return { ok: true, document, doc: replaceIncomingDocument(doc, index, document, now) };
}

export function failIncomingDocumentExtraction(inputDoc, documentId, error, options = {}) {
  const eventId = text(options.eventId, 80) || DEFAULT_EVENT_ID;
  const doc = normalizeIncomingDocumentIntake(inputDoc, { eventId });
  const index = doc.documents.findIndex(record => record.id === documentId);
  if (index < 0) return { ok: false, error: "Document not found.", doc };
  const current = doc.documents[index];
  const extractionVersion = Math.max(0, Math.round(Number(options.extractionVersion) || 0));
  if (current.extractionVersion !== extractionVersion) return { ok: false, stale: true, error: "Document extraction request is stale.", doc };
  const now = iso(options.now) || new Date().toISOString();
  const terminal = options.terminal === true;
  const document = normalizeRecord({
    ...current,
    extractionStatus: terminal ? "failed" : "queued",
    extractionCompletedAt: terminal ? now : null,
    extractionJobId: terminal ? current.extractionJobId : null,
    extractionError: text(error, 1_000) || "Document extraction failed.",
    updatedAt: now
  });
  return { ok: true, terminal, document, doc: replaceIncomingDocument(doc, index, document, now) };
}
...
export function summarizeIncomingDocuments(inputDoc, options = {}) {
  ...
  let bytes = 0;
  let previewReady = 0;
  const extraction = Object.fromEntries(INCOMING_DOCUMENT_EXTRACTION_STATUSES.map(status => [status, 0]));
  ...
  for (const record of doc.documents) {
    ...
    extraction[record.extractionStatus] += 1;
  ...
  return {
    ...
    bytes,
    previewReady,
    extractionReady: extraction.preview_ready + extraction.ready,
    extractionQueued: extraction.queued + extraction.extracting,
    extractionNeedsReview: extraction.needs_review + extraction.failed,
    extraction,
    overdue,
    dueSoon,
    byStatus,
...
export function adminIncomingDocument(record) {
  ...
  const { storageKey: _storageKey, extractionChunks: _extractionChunks, extractionJobId: _extractionJobId, ...safe } = normalized;
  return { ...safe, extractionSupported: incomingDocumentExtractionSupported(normalized) };
}
