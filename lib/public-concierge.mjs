...
const GUIDANCE_STOP_WORDS = new Set(["a", "an", "and", "are", "at", "be", "can", "do", "for", "how", "i", "in", "is", "it", "me", "my", "of", "on", "or", "the", "there", "to", "what", "where", "with"]);

function guidanceMatch(question, guidance) {
  const queryTokens = normalizedQuestion(question).split(" ").filter(token => token.length > 2 && !GUIDANCE_STOP_WORDS.has(token));
  let selected = null;
  for (const item of Array.isArray(guidance) ? guidance : []) {
    const keywords = (Array.isArray(item?.keywords) ? item.keywords : []).map(normalizedQuestion).filter(Boolean);
    const searchable = normalizedQuestion(`${item?.category || ""} ${item?.question || ""} ${item?.answer || ""} ${keywords.join(" ")}`);
    const phraseScore = Math.max(0, ...keywords.map(keyword => aliasScore(normalizedQuestion(question), keyword)));
    const tokenScore = queryTokens.filter(token => (` ${searchable} `).includes(` ${token} `)).length;
    const score = phraseScore * 3 + tokenScore;
    if (score >= 2 && (!selected || score > selected.score)) selected = { item, score };
  }
  return selected?.item || null;
}

function guidanceAnswer(question, parsedTopic, context) {
  const item = guidanceMatch(question, context.bootstrap?.guidance);
  if (!item) return null;
  const topic = ["accessibility", "parking", "family", "tickets"].includes(parsedTopic)
    ? parsedTopic
    : normalizedQuestion(item.category).replaceAll(" ", "-") || "guidance";
  return result(item.answer, topic, {
    sources: [source(item.id, item.sourceLabel, item.sourceUrl, item.sourceCheckedAt)],
    suggestions: [
      "What should I bring?",
      "What accessibility support is available?",
      "Where should I park?",
      "What is the pet policy?"
    ].filter(suggestion => normalizedQuestion(suggestion) !== normalizedQuestion(question))
  });
}
...
function scheduleAnswer(context) {
  ...
  return result(`${guide.name || "Texas SandFest"} runs ${guide.dateRange || "on dates to be announced"}, ${guide.hours || "with hours to be announced"}, at ${guide.location || "the Port Aransas beach"}.${scheduleCopy}`, "schedule", {
    ...
    sources: [source("schedule", "Published event schedule", "#schedule", guide.lastUpdated), guideSource(guide)],
...
function volunteerAnswer(context) {
  const guide = context.bootstrap?.guide || {};
  const program = guide.volunteer || {};
  const informationUrl = safeHref(program.informationUrl, "#volunteer");
  const informationSource = source("volunteer-information", "Official volunteer information", informationUrl, program.sourceCheckedAt);
  if (!program.informationUrl) {
    return result(`Current volunteer information is not published yet. Contact ${publicContact(guide)} before making plans.`, "volunteer", {
      confidence: "low",
      escalated: true,
      sources: [guideSource(guide)],
      suggestions: ["How do I contact SandFest?", "When is SandFest?"]
    });
  }
  if (program.registrationStatus === "open" && program.registrationUrl) {
    return result("Volunteer registration is open. Review the official volunteer guidance, then use the current registration provider to choose an available role and shift.", "volunteer", {
      sources: [
        informationSource,
        source("volunteer-registration", "Current volunteer registration", program.registrationUrl, program.sourceCheckedAt)
      ],
      suggestions: ["When is SandFest?", "What accessibility guidance is available?", "How do I contact SandFest?"]
    });
  }
  const stateCopy = {
    paused: "Volunteer registration is temporarily paused.",
    closed: "Volunteer registration is closed for this event.",
    upcoming: "Current-event volunteer registration has not opened yet."
  }[program.registrationStatus] || "Current-event volunteer registration is not open.";
  const note = text(program.note, 500);
  return result(`${stateCopy}${note ? ` ${note}` : ""} Review the official volunteer page for current guidance.`, "volunteer", {
    confidence: "high",
    sources: [informationSource],
    suggestions: ["When is SandFest?", "How do I contact SandFest?", "What accessibility guidance is available?"]
  });
}
...
export function answerPublicConcierge(questionInput, context = {}) {
  ...
  const governedGuidance = parsed.topic === "emergency" ? null : guidanceAnswer(parsed.question, parsed.topic, context);
  if (governedGuidance) return { ok: true, ...governedGuidance };
  const answer = {
    tickets: ticketAnswer,
    schedule: scheduleAnswer,
    sponsor: sponsorAnswer,
    vendor: vendorAnswer,
    volunteer: volunteerAnswer,
