...
export function createFileStorage({ root, configPaths }) {
  ...
  const ORDER_DIR       = path.join(root, "data", "processed", "orders", "pending");
  const PAYMENT_DIR     = path.join(root, "data", "processed", "orders", "payment-events");
  const FULFILLMENT_DIR = path.join(root, "data", "processed", "orders", "fulfillment");
  const AUDIT_DIR       = path.join(root, "data", "processed", "admin-audit");
  ...
  return {
    ...
    orders: {
      async write(order, { prefix = "checkout" } = {}) {
        const filePath = path.join(ORDER_DIR, timestampedFileName(prefix, order.id));
