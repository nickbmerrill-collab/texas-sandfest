...
import { emptyBudgetControl } from "./budget-control.mjs";
import { emptyGuestServices } from "./guest-services.mjs";
import { emptySculptorRoster } from "./sculptor-roster.mjs";

export const ROLLOVER_DOCUMENT_KEYS = CURRENT_EVENT_OPERATIONAL_DOCUMENT_KEYS.flatMap(key => (
  key === "passportHunt" ? [key, "passportCompletions"] : [key]
));
...
function resetDocument(key, input, { eventId, year, guide, now }) {
  ...
  switch (key) {
  case "guestServices":
    return { ...emptyGuestServices(eventId), lastUpdated: now };
  case "budgetControl":
    return { ...emptyBudgetControl(eventId), lastUpdated: now };
  case "fleet":
    return {
      ...doc,
      eventId,
  ...
  case "sculptorRoster":
    return { ...emptySculptorRoster(eventId), lastUpdated: now };
  case "passportHunt": {
...
function isEmptyTargetDocument(key, doc, targetEventId) {
  if (key === "sculptorRoster") {
    return doc?.meta?.eventId === targetEventId
      && doc?.meta?.publicationStatus === "unpublished"
      && (!Array.isArray(doc.sculptors) || doc.sculptors.length === 0)
  ...
  if (key === "budgetControl") {
    return ["budgetLines", "expenses"].every(field => !Array.isArray(doc[field]) || doc[field].length === 0);
  }
...
export function planEventRollover({ fromEventId, toEventId, guide, documents, now = new Date().toISOString() } = {}) {
  ...
  const mismatches = ROLLOVER_DOCUMENT_KEYS
    ...
    .map(key => ({ key, eventId: operationalDocumentEventId(key, sourceDocuments[key]) }))
  ...
  return {
    ...
    summary: {
      ...
      resetSculptors: sourceDocuments.sculptorRoster.sculptors?.length || 0,
      ...
      resetBudgetLines: sourceDocuments.budgetControl.budgetLines?.length || 0,
      resetExpenses: sourceDocuments.budgetControl.expenses?.length || 0,
