...
export function smsConfigFromEnv(env = process.env) {
  ...
  const callbacksValid = validUrl(statusCallbackUrl, { httpsOnly: production })
    ...
    && validUrl(marketingInboundWebhookUrl, { httpsOnly: production });
  ...
  const reason = !enabled
    ...
    : !hasAuth
      ...
      : !hasSender
        ...
        : !callbacksValid
          ? `TWILIO_STATUS_CALLBACK_URL, TWILIO_SAFETY_INBOUND_WEBHOOK_URL, and TWILIO_MARKETING_INBOUND_WEBHOOK_URL must be valid${production ? " HTTPS" : ""} URLs.`
          : !apiOriginValid
            ? production
