...
Local development defaults to atomic JSON documents and a filesystem queue for inspectable test data. Production mode supports Postgres row locking, append-only audit/event tables, platform documents, and the durable job queue exercised by `npm run test:postgres`. Annual Postgres rollover archives append history and replaces all 15 governed ledgers inside one serializable transaction; the integration suite injects a late document-write failure and proves that neither the snapshot nor any ledger can partially commit. A launch deployment must set `SANDFEST_DATABASE_URL`, run with Postgres storage, and keep API and worker processes on the same database; file mode, including its compensating rollover restore, is not a production data plane for 100,000 visitors.
...
The production Blueprint uses paid managed Postgres with PITR and an encrypted persistent disk with daily snapshots. Recovery is treated as unproven until isolated restores of both data planes succeed. `/ready` requires recent timestamps for both drills; `npm run recovery:verify` checks the current schema, governed configuration, 15 named operational ledgers, and exact annual context in one repeatable-read transaction and emits a content-bound aggregate database manifest hash. `npm run recovery:verify:assets` then proves every Postgres-referenced upload by size and SHA-256 and emits a deterministic aggregate asset manifest hash.
