...
1. **Unified revenue ledger** — `lib/revenue.mjs` now projects audited sponsor/vendor receipts and reversals directly from the current-event operations ledger, merges explicitly event-scoped Stripe/Eventeny/Square imports, excludes stale or unscoped rows, and serves source provenance plus category/source/refund/reconciliation KPIs through role-guarded `GET /api/admin/revenue`. Finance can use the operations console or `POST /api/admin/revenue/import` to preview and atomically commit provider settlement CSVs with strict amounts, row exceptions, duplicate/replay protection, bounded history, and audit evidence. Direct provider API synchronization still requires credentials.
...
5. **Fleet/asset checkout** — `lib/fleet.mjs` (normalize + check-out/in transitions) + seed `data/processed/fleet.json` + role-guarded admin API (`GET /api/admin/fleet`, resolve-qr, checkout, checkin, locations) with `fleet:read`/`fleet:write` + web ops panel + native iOS Admin → Fleet tab (QR via existing `QRScannerView`, authenticated board API reads and mutations with no local mutation fallback). QR payload: `tsf:asset:<id>`.
6. **Volunteer coverage mirror** — `lib/volunteers.mjs` + `lib/volunteer-import.mjs` + `GET /api/admin/volunteers` / `.../coverage` (`volunteers:read`) + preview-gated `POST /api/admin/volunteers/import` (`volunteers:write`) + web staffing workspace (zone fill, understaffed shifts, hours, and recent imports). Roster, shift, and hour exports reconcile atomically with current-event, mirror-version, consent, assignment, and replay gates. Buy VolunteerLocal; mirror only.
7. **Consent-safe Twilio safety SMS lifecycle** — separate checkout channels (unchecked by default), explicit operator confirmation, durable consent-reference jobs, immediate pre-send revalidation, signed delivery callbacks, channel-isolated STOP/START/HELP handling, aggregate admin evidence, and stale-alert cancellation. The integration is implemented and tested locally; real sender approval, credentials, callback configuration, and launch-volume certification remain external production gates. See [`sms-safety.md`](sms-safety.md).
...
10. **Public booth/vendor map** — `lib/booths.mjs` + `lib/booth-import.mjs` + preview-gated `POST /api/admin/booths/import` (`booths:write`) + the Booth workspace. Eventeny booth/vendor exports reconcile through annual-event, exact-file, current-mirror, privacy, compliance, assignment, transaction, audit, and replay gates before approved public pins are derived.
