...
3. Click **Deploy Blueprint** only after approving the displayed recurring cost. The first deploy builds two services and provisions the data plane. After the board presentation, add one provider's complete variables to both API and worker where needed, update `render.yaml` so the configuration remains reproducible, enable only that capability, and rerun its acceptance gate before moving to the next provider.
