...
3. The API reserves one active checkout for the current server-computed balance and reuses it on retries. If Stripe created the session but the response was lost, the next request keeps the original SandFest checkout ID and Stripe idempotency key, recovers the provider session, and prevents a late failure from overwriting the recovered link.
...
- Interrupt a partner Checkout Session response after Stripe accepts it, retry from the private portal, and confirm the same SandFest checkout ID and Stripe idempotency key recover the original provider session.
