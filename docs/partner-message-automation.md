...
If the worker loses a trustworthy response after provider submission begins, the message enters `delivery_unknown` instead of failed or queued. That state reserves campaign capacity and blocks retry, dismissal, replacement drafts, and later sequence steps. An administrator must reconcile it in Operations with a verification note: `confirmed_sent` also requires the provider message ID and records accepted delivery evidence, while `confirmed_not_sent` clears stale provider evidence and releases an explicit retry. If consent, task, milestone, workflow, or campaign changes made the message obsolete while verification was pending, confirmed non-delivery dismisses it instead. Reconciliation audits retain the outcome and presence of provider proof without recipient addresses, message text, or idempotency keys.
