...
const commands = new Set(["start", "restart", "status", "stop"]);

if (!commands.has(command) || unknown.length) {
  console.error("Usage: node scripts/board-service.mjs <start|restart|status|stop>");
  process.exit(1);
}
...
function printReady({ session, report }) {
...
async function main() {
  ...
  const ready = await waitForReady({
    ...
    requireReplacement: result.action === "restarted" || result.action === "started"
  });
